...
Bu rehber, bu çalışma ağacındaki Zed/GPUI koduna göre doğrulanmıştır
(`git rev-parse --short HEAD`: `c8f09caee4`; önceki hazırlanma commit'i:
`4b23564f36`). Amaç, Zed içinde yeni bir pencere,
...
`4b23564f36..c8f09caee4` aralığında GPUI çekirdeğinde bu rehberin anlattığı API
yüzeyini değiştiren bir fark yoktur. Rehberi etkileyen değişiklikler Zed
workspace ve collab/titlebar tarafındadır: multi-workspace artık sidebar açık
olmasa da AI/agent ayarları etkinse workspace'leri retain eder; bu etkinlik
koşulu true'dan false'a geçince tek workspace'e collapse eder; aktif pane
kaldırıldığında `active_pane` mutlaka kalan pane'e güncellenir; collab/titlebar
tarafında client `User` kimliği legacy protokol sınırları için `legacy_id`
alanından okunur. Bu noktalar ilgili bölümlere işlenmiştir.
