...
Bu bölüm, mevcut Zed `ui` katmanındaki bileşenleri GPUI temeliyle birlikte anlatır. Amaç yalnızca hangi component'in hangi constructor ile üretildiğini listelemek değildir. Bir ekranda hangi parçanın neden seçildiğini, state'in nerede tutulduğunu ve kullanıcı etkileşiminden sonra render'ın nasıl güncellendiğini anlaşılır hale getirmek de amaçlanır.
...
- Kullanıcının gördüğü bir state değiştiğinde `cx.notify()` çağrısı yaparsın. Aksi halde model güncellense bile ekrandaki görünüm aynı kalabilir.
