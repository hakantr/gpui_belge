...
Okurken şu ayrımı akılda tutman süreci kolaylaştıracaktır:
...
- Hazır bir Zed bileşeni ihtiyacını karşılıyorsa öncelikle onu kullanmayı tercih etmen önerilir. Ham GPUI primitive'lerine genellikle özel düzen (layout), özel çizim, sanallaştırma veya hazır bileşenin kapsamadığı bir etkileşim gerektiğinde başvurman gerekir.
...
- Kullanıcının gördüğü bir durum değiştiğinde `cx.notify()` çağrısı yapman gerekir. Aksi halde model güncellense bile ekrandaki görünümün eski kalması olasıdır.
