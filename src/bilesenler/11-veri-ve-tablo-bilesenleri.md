...
- `.map_row(...)`, tablonun ürettiği `Stateful<Div>` satır container'ını alır. Bu sayede seçili satır, hover state'i, sağ tık veya özel tıklama davranışı gibi ek davranışlar eklemek mümkün hale gelir.
...
Sabit yükseklikli büyük bir liste için tablo `.uniform_list(...)` ile kurarsın. Burada satır sayısı view içinde tutulur ve sadece görünür range render edilir:
...
Değişken yükseklikli satırlarda ise `ListState` ile birlikte `.variable_row_height_list(...)` kullanırsın. Veri her değiştiğinde state'in `reset(...)` veya uygun bir `splice(...)` ile güncellenmesi gerekir:
...
- `ColumnWidthConfig::table_width(window, cx)`: tablo container'ının kullanacağı toplam genişliği döndürür. `Resizable` modunda kolon genişliklerini rem boyutuyla piksele çevirip toplar; diğer modlarda konfigürasyondaki sabit tablo genişliğini kullanır.
...
- `Resizable` için associated bir constructor yoktur; enum variant'ı doğrudan `ColumnWidthConfig::Resizable(entity)` biçiminde kullanırsın.
...
- `TableRenderContext::for_column_widths(column_widths, use_ui_font)`:
  ...
  - `use_ui_font`: `true` olduğunda hücre içeriği `text_ui(cx)` ile çizilir; `false` olduğunda font ailesi parent'tan miras alırsın. `Table::no_ui_font()` ile kapatılan davranışla aynıdır. CSV preview monospace bir görünüm için bu değeri `false` verir.
...
- `HeaderResizeInfo::from_resizable(&columns_state, cx)`.
  - `resize_behavior: TableRow<TableResizeBehavior>` public alanı, header hücresinin resizable olup olmadığını okumak için kullanırsın. İlgili kolon state'i public bir alan değildir; reset ve state update için `reset_column(...)` çağırırsın.
...
| `HeaderResizeInfo` | Header hücresinin resize davranışını ve kolon reset akışını bağlamak için redistributable veya resizable state'ten üretilir. |
| `bind_redistributable_columns` | Redistributable kolon state'ini drag preview ve drop commit davranışıyla container'a bağlar. |
...
| `render_table_row` | Satır index'i, hücreler ve `TableRenderContext` üzerinden tablo gövde satırını render eder. |
...
- `render_redistributable_columns_resize_handles(...)`, kolon state'inden divider'ları üretir; container'ın `relative()` olması handle yerleşimini çok daha öngörebilirsin hâle getirir.
- `render_table_header(...)` içinde çift tıklama ile kolon reset davranışı `HeaderResizeInfo` üzerinden bağlanır.
- Header ve row için aynı `TableRenderContext` genişlik modelinin kullanman gerekir; aksi halde hücreler hizalanmaz.
...
Satır seçimi için `map_row(...)` ile satır container'ının üzerinde özel bir görsel state uygulayabilirsin. Aşağıdaki örnek seçili satıra arka plan rengi atar:
