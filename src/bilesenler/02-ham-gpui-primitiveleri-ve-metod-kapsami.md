...
Kimlik ve odak ailesinde `group(...)`, `id(...)`, `track_focus(...)`, `tab_stop(...)`, `tab_index(...)`, `tab_group()` ve `key_context(...)` yer alır. Bu metotlar elementin dispatch ağacında nasıl bulunacağını, klavye gezinmesine katılıp katılmayacağını ve hangi keybinding bağlamında çalışacağını belirler. Görsel odak stilleri için `focus(...)`, `in_focus(...)` ve `focus_visible(...)` kullanılır; `focus_visible` özellikle son girdinin klavye olduğu durumlarda odak halkasını göstermeye yarar.

Fare ve pointer ailesi `on_mouse_down(...)`, `on_mouse_up(...)`, `on_mouse_move(...)`, `on_mouse_down_out(...)`, `on_mouse_up_out(...)`, `on_mouse_pressure(...)` ve bunların capture/any varyantlarından oluşur. Normal varyantlar elemente gelen olayı işler; capture varyantları  dispatch'in erken aşamasında devreye girer; `*_out` metotları menü veya popup kapatma gibi element dışı tıklama akışları için kullanılır. Scroll ve hareket olayları `on_scroll_wheel(...)`, `on_pinch(...)` ve `capture_pinch(...)` ile bağlanır.
