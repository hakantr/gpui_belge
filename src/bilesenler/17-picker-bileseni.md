...
- [x] Doğrulanan duyarlı yerleşim davranışı: kalıcı yön `PreviewLayout::Right` iken sağ yerleşim minimum genişliğe sığmıyorsa picker gövdesi geçici olarak alt alta çizilir; kalıcı yön ve ölçü anahtarı `Right` kalır.
...
Varsayılan ölçüler iki sabit üzerinden okunur: `DEFAULT_MODAL_WIDTH = Rems(34.0)` ve `DEFAULT_MODAL_MAX_HEIGHT = Rems(24.0)`. `shape::Centered::simple()` sade picker için bu değerleri kullanır. Preview layout'u gizliyken aynı sade ölçü korunur; preview sağda veya altta açıldığında `default_shape_for_layout(...)` daha geniş preview odaklı yerleşimi seçer. Saklanan yön `Right` olduğunda picker, sınırlandırılmamış toplam genişliği sonuç ve preview bölümlerinin sağ yerleşimdeki minimum genişlikleri toplamına eşit veya bu toplamdan küçükse gövdeyi `Below` düzeninde çizer. Pencere yeniden genişlediğinde gövde kendiliğinden sağ yerleşime döner. Footer bu geçici durumda `DiffSplitAuto` ikonunu gösterirken sağ yön düğmesini seçili tutar.

Boyut kalıcılığı `KeyValueStore` içinde `pickers_v2` ad alanına yazılır; anahtarlar delegate `name()` değeri ile preview layout adını birlikte taşır (`hidden`, `below`, `right`, `none`). Preview konumu değiştirildiğinde son layout tercihi de aynı delegate adıyla saklanır; preview sonradan kapansa bile bir sonraki preview'lu açılış bu son yön bilgisinden devam eder. Otomatik alt alta çizim saklanan `Right` tercihini değiştirmez. Bu görünümde yapılan yeniden boyutlandırma ve bölücü ayarları da `Right` ölçü anahtarına yazılır; böylece pencere genişlediğinde aynı sağ yerleşim ölçüleri geri kullanılır.
...
- `PreviewSource::Symbol(Symbol)`: Proje sembolünden buffer'ı açar; highlight konumu sembolün aralığından türetilir.
- `PreviewSource::Message(HighlightedText)`: Dosya veya buffer yerine merkezde vurgulu bir açıklama gösterir.
- `PreviewUpdate::from_path(...)`, `PreviewUpdate::from_buffer(...)`, `PreviewUpdate::from_symbol(...)` ve `PreviewUpdate::message(...)`: En yaygın update biçimlerini kısa yoldan üretir.
...
| `PreviewLayout` | `Hidden`, `Below`, `Right` | Preview panelinin saklanan yönünü belirten yeniden ihraç edilen layout enum'ıdır. `Right`, kullanılabilir genişlik minimum toplamı karşılamadığında geçici olarak alt alta çizilebilir; enum değeri ve kalıcılık anahtarı değişmez. |
...
| `picker::preview::Update` | `source`, `match_location`; `from_path`, `from_buffer`, `from_symbol`, `message` | Preview güncellemesinin asıl modül içi tip adıdır; crate kökü aynı tipi `PreviewUpdate` adıyla yeniden dışa aktarır. |
| `PreviewUpdate` | `source`, `match_location`; `from_path`, `from_buffer`, `from_symbol`, `message` | Delegate'in seçili eşleşme için preview içeriğini güncelleme sözleşmesidir. |
...
- Proje sembolleri picker'ı, seçili sembol için `PreviewUpdate::from_symbol(symbol)` üreterek buffer yüklenince sembol aralığına kaydırılan preview gösterir. Bu yol, çağıranın ayrıca `MatchLocation` hesaplamasını gerektirmez.
...
- Otomatik alt alta çizim `PreviewLayout` durumunu değiştirmez. `preview_layout_changed(true)` ile yatay satır sunumuna geçen delegate veya `PreviewBackend::render(layout, ...)` içindeki yön dalları, dar pencerede de saklanan `Right` değerini görür. Duyarlı gövde yerleşimi picker tarafından yönetilir; backend'in fiziksel kapsayıcı yönünü yalnız bu enum'dan çıkarmaması gerekir.
