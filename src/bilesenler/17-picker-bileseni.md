...
- `selected_index` sınır dışı kalırsa liste yine `match_count()` ile sürüldüğü için satırlar çizilir; yalnız hiçbir satır seçili görünmez, picker boş çizilmez. Yine de seçimin kaybolmaması için `update_matches` sonrasında `match_count == 0` durumu ayrı ele alınmalı, diğer durumlarda index `min(match_count - 1, selected_index)` ile yenilenmelidir.
