...
- Buton toolbar'ları, metadata satırları, icon ile label kombinasyonları ve panel içerik düzenleri için en sık başvurduğun iki helper'dır.
...
- Sadece tek bir stil ihtiyacı varsa, doğrudan `div()` kullanmak okuyucu açısından daha açık bir tercih olabilir; gereksiz bir helper kullanmak yerine niyetini açıkça yazarsın.
...
- Her iki helper da yalnızca layout stilini ayarlar. Gap, width, overflow ve responsive davranışı ayrıca tanımlarsın; bunlar otomatik olarak verilmez.
...
- Uzun metin taşıyan h-flex satırlarında parent elemana `.min_w_0()` ve label elemanına `.truncate()` eklemen beklenir; aksi halde flex algoritması metni kısaltmak yerine satırı taşırabilir.
...
- `h_group*` helper'ları `items_center()` eklemez. Satırdaki elemanların dikey hizası bir konu olarak öne çıkıyorsa `.items_center()` veya `.items_start()` çağrılarını elle eklemen gerekir.
...
İkinci örnek, dar bir metadata bloğunu dikey istif olarak kurar:
...
- Group helper'larını iç içe fazla kullanmak layout'u zamanla belirsizleştirir. Ana container için `h_flex` veya `v_flex`, küçük alt kümeler için group helper kullanımı, bu hiyerarşinin okunabilir kalmasını sağlar.
...
| Konu | Üyeler | Not |
|---|---|---|
| `Divider` | `description`, `scope` | Trait impl üzerinden gelen public üyelerdir; çoğu dönüşüm, render, builder veya standart trait köprüsüdür. |
...
- Tablo ya da listede satırları ayıran bir semantic separator gerekiyorsa, ilgili component'in kendi border veya separator davranışını kullanırsın.
...
- Vertical divider'ın görünür olabilmesi için parent container'ın belirli bir yüksekliği olması veya yüksekliğin içerikten otomatik türemesi gerekir; aksi halde dikey çizgi 0 boy alıp kaybolur.
...
- Divider bir layout aracı değildir; tamamen görsel bir ayrım için vardır. Çok sık kullandığında UI hızla kalabalıklaşır; section hiyerarşisini öncelikle spacing ve başlıklar üzerinden kurman daha sade bir sonuç verir, divider'ı en son ihtiyaç hâline getirirsin.
...
Inline metadata için ise küçük bir ikon, bir branch adı ve bir ahead sayacı gibi yan yana duran küçük parçaları bir `h_group_sm()` içinde toplaman yeterlidir. Bu örnek, group helper'larının küçük ölçekli inline kompozisyonlarda nasıl rahat bir okuma sağladığını ortaya koyar:
