...
| `ApplicationMenu` | İstemci tarafı uygulama menüsü (Dosya, Düzen, Görünüm…). Yalnızca platform menüsü kullanılmadığında çizilir. |
...
Bu parçalardan yalnızca `TitleBar`, `collab` modülü ve birkaç eylem (`ToggleUserMenu`, `ToggleProjectMenu`, `SwitchBranch`, `SimulateUpdateAvailable`, `UseClassicLayout`, `UseAgenticLayout`) ile `restore_banner` crate dışına açık durumdadır. `ApplicationMenu`, `UpdateVersion`, `OnboardingBanner`, `PlanChip` ve `TitleBarSettings` özel modüllerde kalır; `TitleBar` bunları kendi içinde çocuk entity veya yardımcı olarak kullanır.
...
Bu çift yüzey, iki katmanın bağımlılık yönünü de gösterir: `title_bar`, `platform_title_bar`'a bağımlıdır; tersi ise mümkün değildir. Ürün başlığı platform kabuğunu kullanır, platform kabuğu ürün başlığını tanımaz.
...
`TitleBar`, `PlatformTitleBar` entity'sini **güçlü** (strong reference) olarak sahiplenir. Her render geçişinde önce ürün çocuklarını hazırlar, sonra bunları platform kabuğuna teslim eder. Teslim iki çağrıyla gerçekleştirilir:
...
Bu noktada Platform Üst Barı bölümünde anlatılan kural devreye girer: `PlatformTitleBar`, kendisine verilen çocukları render sırasında tüketir. Bu yüzden `TitleBar`, çocuk listesini her render geçişinde yeniden kurar ve yeniden teslim eder. Tek seferlik bir teslim yeterli olmaz.
...
Port hedefinde bu sınır korunduğunda, platform kabuğunu hiç değiştirmeden ürün başlığını tamamen yeniden tasarlamak mümkündür. Sınır bulanıklaşırsa — örneğin proje adı mantığı platform kabuğuna sızarsa — kabuk tek bir uygulamaya kilitlenir ve yeniden kullanılamaz hale gelir.
...
Bu iskelet, Zed'in `TitleBar` render'ının çekirdeğiyle aynı sözleşmeyi taşır. Sonraki bölümlerde bu çekirdeğin içine ürün parçaları tek tek yerleştirilecektir.
