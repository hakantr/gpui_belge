...
| Parça | Rol |
| :-- | :-- |
| `TitleBar` | Ürün başlığının ana entity'si. Tüm parçaları birleştirir ve her render'da `PlatformTitleBar`'ı besler. |
| `ApplicationMenu` | İstemci tarafı uygulama menüsü (File, Edit, View…). Yalnız platform menüsü kullanılmadığında çizilir. |
| `collab` modülü | Aktif çağrı sırasındaki mikrofon, deafen, ekran paylaşımı ve katılımcı listesi yüzeyi. |
| `UpdateVersion` | Otomatik güncelleme durumunu (checking/downloading/installing/updated/errored) gösteren entity. |
| `OnboardingBanner` | Özellik bayrağına bağlı duyuru banner'ı (güncel kullanım: Skills duyurusu). |
...
Bu parçalardan yalnız `TitleBar`, `collab` modülü ve birkaç action (`ToggleUserMenu`, `ToggleProjectMenu`, `SwitchBranch`, `SimulateUpdateAvailable`) ile `restore_banner` crate dışına açıktır. `ApplicationMenu`, `UpdateVersion`, `OnboardingBanner`, `PlanChip` ve `TitleBarSettings` private modüllerde kalır; `TitleBar` bunları kendi içinde child entity veya yardımcı olarak kullanır.

## 2. `platform_title_bar` köprüsü

`title_bar` crate'i, platform kabuğunu kendi kök yüzeyinden yeniden dışa aktarır. Böylece Zed içindeki tüketiciler hem platform sekmesi action'larını hem de `PlatformTitleBar` tipini `title_bar` üzerinden de alabilir:
...
`TitleBar`, `PlatformTitleBar` entity'sini **strong** olarak sahiplenir. Her render geçişinde önce ürün child'larını hazırlar, sonra bunları platform kabuğuna teslim eder. Teslim iki çağrıyla olur:

- `set_button_layout(...)` ile Linux/FreeBSD pencere butonu yerleşimini iletir (ayardan gelen değer).
- `set_children(...)` ile başlıkta görünecek ürün element'lerini iletir.

Bu noktada Platform Üst Barı bölümünde anlatılan kural devreye girer: `PlatformTitleBar`, kendisine verilen child'ları render sırasında tüketir. Bu yüzden `TitleBar`, child listesini her render'da yeniden kurar ve yeniden teslim eder. Bir defalık teslim yeterli olmaz.
...
| Karar | Sahibi |
| :-- | :-- |
| Pencere sürükleniyor mu, butonlar nerede, native sekme açık mı? | `PlatformTitleBar` |
| Başlıkta proje adı, branch, kullanıcı menüsü görünüyor mu? | `TitleBar` |
...
Kendi uygulamanda aynı köprüyü kurarken iki entity tutarsın: platform kabuğunu temsil eden `PlatformTitleBar` (veya port karşılığın) ve onu besleyen ürün başlığı entity'si. Ürün entity'si, her render'da kendi child grubunu hazırlar ve platform kabuğuna teslim eder:
