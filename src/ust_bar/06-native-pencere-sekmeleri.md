...
başlık çubuğunun en karmaşık parçasıdır. Bu yüzden ayrı bir aşama
olarak ele alınır. Burada kararlar tek başına verilmez; controller
state'i, platform çağrıları ve drag/drop hedefleri birlikte düşünülür.
Bu üç parça ayrı ayrı yazılırsa aralarında uyumsuzluk çıkması çok
kolaydır.
...
**Burada kritik bir binding farkı vardır.** Bu action'lar
`workspace.register_action(...)` ile değil,
**`workspace.register_action_renderer(...)`** ile bağlanır
(`system_window_tabs.rs:97`). İki API arasındaki fark hem zamanlama hem
de kapsam açısından önemlidir:
...
Pratik sonuç şudur: `ShowNextWindowTab` action'ı yalnızca workspace
içinde birden fazla tab açıkken çalışır. Tek tab varken çağrılsa bile
etki üretmez. Çünkü runtime'da action map o frame için koşula göre
yeniden kurulur. Port hedefinde "Workspace" kavramı birebir yoksa bu
desen aynen taşınamayabilir. Yine de "her render'da action handler'ları
koşullu olarak yeniden bağla" prensibi korunmalıdır. Aksi halde tek tab
varken bile sekme dolaşma action'ı çalışıyormuş gibi görünür ve
davranışı test etmek zorlaşır.

Ayrıca bu API'nin bir kısıtı daha vardır: `register_action_renderer`
bağlandığı `Workspace` entity'sine kilitlenir. Bu yüzden **bu
action'lar yalnızca bir workspace içindeyken dispatch edilebilir**.
Workspace bağlamı dışındaki pencereler, örneğin standalone bir settings
penceresi, bu action'ları hiç görmez.

### Sekme butonları

`SystemWindowTabs` içindeki sekme kapatma yollarının tamamı ortak olarak
**`workspace::CloseWindow` sabit action'ını** dispatch eder. Bu yolların
kaç tane olduğunu görmek için `Box::new\(CloseWindow\)` deseniyle
yapılan awk taraması altı farklı çağrı noktası çıkarır:
...
Bu altı yolun her birinde **aynı sabit action dispatch edilir**. Dış
crate'in verdiği `close_action` prop'u bu kapatma yollarına ulaşmaz.
Dışarıdan gelen close action yalnızca Linux tarafındaki
`LinuxWindowControls`/`WindowControl` zincirinde kullanılır. Port
hedefinde sekme kapatma davranışı farklılaştırılacaksa, bu altı çağrı
noktasının her biri ayrı ayrı ele alınmalıdır. Tek bir flag açıp
kapatmak hepsini aynı anda değiştirmeye yetmez.

**Cross-window dispatch deseni**, yani
`handle.update(cx, |_, window, cx| { ... })` çağrı zinciri, bu sekme
yollarının dördünde merkezi rol oynar. Bu yollar çağrı anında
"**hedef pencere mevcut pencere mi?**" sorusunu sorar. Hedef başka bir
pencereyse
...
`handle\.update\(` deseninin awk taraması altı çağrı noktası gösterir:
...
Çünkü `update()` fonksiyonu `Result<R, ()>` döndürür; çağrı sırasında
pencere zaten kapanmış olabilir. Böyle bir durumda sonuç bilinçli
olarak yutulur ve hata fırlatılmaz.

Port hedefinde aynı deseni karşılamak için üç şey birlikte sağlanmalıdır.
Her tab metadata yapısı bir handle veya proxy taşımalıdır. Cross-window
işlemler bu proxy üzerinden ilgili pencerenin context'ine girip işi
orada yapmalıdır. Proxy çağrısı da **fail-soft** davranmalı; hedef
pencere ortadan kalkmışsa sessizce geçilmelidir. Bu üç kural birlikte
uygulanmazsa kapanmış pencerelere yapılan çağrılar uygulamayı
çökertebilir.

**Conditional Option idiom'u**, `platform_title_bar.rs` içinde
`248-255` ve `299-306` aralıklarında geçer. Sol ve sağ kontroller şu
desenle dahil edilir:
`show_X_controls.then(|| render_X_window_controls(...)).flatten()`.

Bu desen şöyle çalışır: `bool::then(|| fn)` ifadesi boolean `true` ise
`Some(fn())`, `false` ise `None` döner. `render_X_window_controls`
fonksiyonu zaten `Option<AnyElement>` döndürdüğü için dış sarmal
`Option<Option<...>>` olur ve `.flatten()` ile tek seviye `Option`'a
iner. Burada gözden kaçabilecek bir yan etki vardır. `then` closure'u,
boolean `true` olduğunda gövdesini çalıştırır ve gövde içinde **clone
işlemi** gerçekleşir. İleride bahsedilecek `boxed_clone` zincirindeki
adım 2 ve 3'ün her render'da çalışmasının nedeni budur. Aynı sonuç daha
açık şekilde `if show_X { Some(render_X(...)) } else { None }`
ifadesiyle de yazılabilir. `then().flatten()` formu yalnızca daha
kısadır; ek bir avantaj sağlamaz.

Sağ tık menüsü `ui::right_click_menu(ix).trigger(...).menu(...)`
builder zinciriyle kurulur (`system_window_tabs.rs:279-343`). Yapı
şudur:
...
`move |window, cx| {...}` closure'u alır. Her closure gövdesinde ortak
olarak `Self::handle_right_click_action(cx, window, &tabs_clone, |tab|
predicate, |window, cx| body)` çağrısı yapılır.

Buradan ilginç bir bellek davranışı doğar: `tabs` vec'i **dört defa
clone'lanır** (`tabs.clone()`, `other_tabs.clone()`, `move_tabs.clone()`,
`merge_tabs.clone()` — `system_window_tabs.rs:283-286`). Bunun nedeni
her closure'ın kendi owned kopyasına ihtiyaç duymasıdır. Reference
olarak paylaşmak closure lifetime'larıyla çakışır. Port hedefinde aynı
builder kalıbı kullanılabilir:
`right_click_menu(id).trigger(trigger_fn).menu(menu_builder_fn)`. Entry
ekleme işi ise `menu()` callback'i içinde `ContextMenu::build` ile
yapılır.
...
ihtimalle gerekir. Bağımsız bir uygulamada bu action genellikle
...
zincirinden değil, daha önce çalışan başka bir yoldan beslenir.
`zed::build_window_options(...)` fonksiyonu içindeki
`tabbing_identifier` alanı bu görevi üstlenir (`zed/src/zed.rs:331-373`).
GPUI'nin pencere bootstrap aşaması ise platformun `tab_bar_visible()`
ve `tabbed_windows()` çağrılarının sonucunu doğrudan controller'a işler
(`gpui/src/window.rs:1295-1299`). Daha sonra çalışan
`SystemWindowTabs::init(...)` içindeki `observe_global::<SettingsStore>`
gözlemcisi, `was_use_system_window_tabs` değerini hafızasında tutar.
Yalnızca ayar değiştiğinde çalışır; değer aynıysa hemen erken döner
(`system_window_tabs.rs:55-64`).
...
devre dışı bırakmak controller'ı temizlemekle değil, pencere
identifier'larını geri çekmekle olur.

Render sırasında `SystemWindowTabController`'ın global state'i okunur.
Controller, aktif pencerenin ait olduğu sekme grubunu döndürür. Böyle
bir grup yoksa mevcut pencere tek sekme olarak gösterilir.
...
**Burada önemli bir platform farkı vardır.** `Platform::tab_bar_visible()`
trait'inin default implementasyonu `false` döner
(`gpui/src/platform.rs:658-660`). Bu default'u **yalnızca macOS**
override eder. Bunun sonucu olarak Linux ve Windows'ta yukarıdaki ilk
koşulun ilk parçası daima `true` kabul edilir. Yani sekme çubuğunun
görünürlüğü **tamamen `SystemWindowTabController::is_visible(...)`
state'ine** bağlanır.

`is_visible` fonksiyonu `self.visible == Some(true)` kontrolünü yapar
(`gpui/src/app.rs:395`). Fakat `visible` alanı, `init_visible` veya
`set_visible` çağrılana kadar `None` durumundadır. Dolayısıyla pencere
ilk açıldığında ve `on_toggle_tab_bar` callback'i tetiklenmediği sürece
Linux ve Windows'ta sekme çubuğu **varsayılan olarak gizli** kalır.
macOS dışındaki platformlarda sekme çubuğunun görünmesi için
controller'ın açıkça görünür duruma alınması gerekir. Yalnızca
`tab_bar_visible` çağrısını aramak yetmez. Bu noktayı atlayan bir port,
"neden Linux'ta sekmeler hiç görünmüyor?" sorusuyla çok zaman
kaybedebilir.

macOS'taki native tabbing için `set_tabbing_identifier(Some(...))`
çağrısı yalnızca pencereye identifier yazmaz. Aynı çağrı paralel olarak
`NSWindow::setAllowsAutomaticWindowTabbing:YES` fonksiyonunu da çağırır.
Ters yönde, `None` değeri geldiğinde aynı global izin `NO` yapılır ve
pencerenin tabbing identifier'ı `nil` olur
(`gpui_macos/src/window.rs:1174-1191`). Bu yüzden Zed'in
`SystemWindowTabs::init(cx)` içindeki settings observer'ı yalnız
controller state'ini değil, macOS'un native tabbing politikasını da açıp
kapatır.

**Drag/drop'ta owner ve olay ayrımı.** `DraggedWindowTab` tipinin adını
ve alanlarını mirror etmek tek başına yeterli değildir. Sürükleme ve
bırakma davranışı, olayın hangi alanda tetiklendiğine göre farklı
yollar izler. Kaynaktaki owner ve olay ayrımı şöyledir:
...
Bu farklar nedeniyle native tab portunda drag/drop davranışı yalnızca
`DraggedWindowTab` alanlarını taşımakla çözülmez. Davranış aynı zamanda
**olayın hangi hedefte gerçekleştiğine göre** mirror edilmelidir.
"Drop nerede oldu?" sorusunun cevabına göre üç ayrı dal vardır ve bu
dalların hepsi ayrı ayrı kodlanır.
...
Drag/drop sürecinde `on_drag(DraggedWindowTab, ...)` çağrısının payload'ı
bu struct'tır. Aynı `DraggedWindowTab` tipi `Render` trait'ini de
implement eder. Sürükleme önizlemesi bu struct'ın `title`, `width`,
`is_active`, `active_background_color` ve `inactive_background_color`
alanlarından doğrudan çizilir.

`last_dragged_tab` alanı yalnızca geçici state'tir. Tek amacı sekmenin
tab bar dışına bırakılma ihtimalini yakalamaktır. Başarılı bir
`on_drop` çağrısı tamamlandığında bu alan `None` yapılır. Aksi halde
state sızıntısı oluşur.

Preview render'ı, etiket fontunu aktif tema üzerinden
`ThemeSettings::ui_font` değerinden alır. Yüksekliği de
`Tab::container_height(cx)` ile hesaplar
(`system_window_tabs.rs:498-528`). Drag ghost'u için ayrı ve sabit bir
yükseklik tutulmaz; önizleme gerçek sekmeyle aynı boyda çıkar.

**Controller grup mutasyonları** (`gpui/src/app.rs:417-530`). Bu
fonksiyonların public imzaları yüzeyde basit görünür. Fakat gövdedeki
state algoritması port hedefi için kritiktir. Aşağıdaki tablo her
fonksiyonun state üzerinde tam olarak ne yaptığını gösterir:
...
`select_next_tab` ve `select_previous_tab` fonksiyonları yalnız mevcut
grubun içinde döner. Hedef sekmenin `AnyWindowHandle`'ı üzerinde
`activate_window()` çağırarak o pencereyi öne getirir
(`gpui/src/app.rs:532-563`). Grup değiştirme action'ları ise farklı bir
yoldan, `get_next_tab_group_window` ve `get_prev_tab_group_window`
fonksiyonları üzerinden işler.

Burada dikkat çekici bir nokta vardır: bu iki fonksiyonda grup key
sırası `HashMap` key sırası olduğu için tutarlı bir "önce/sonra" tanımı
yoktur. Kaynak kodunda bu duruma işaret eden bir "next/previous ne
demek?" TODO yorumu da bulunur (`gpui/src/app.rs:326-360`). Port
hedefinde grup geçişleri deterministik olmalıysa bu noktada `HashMap`
yerine sıralı bir yapı tercih edilir.

**Tab genişliği ölçümü**, `system_window_tabs.rs:455-471` aralığında
bulunan ince bir mekanizmadır. Tab bar render'ı, kullanıcıya görünmeyen
bir `canvas` elementi içerir. Bu `canvas` tipi iki callback alır
(`gpui/src/elements/canvas.rs:10-13`):
...
number_of_tabs as f32` formülüyle bir sekme genişliği hesaplanır.
Ardından bu değer `entity.update(cx, |this, cx| {
this.measured_tab_width = width; cx.notify() })` çağrısıyla state'e
yazılır.

Bu yapının döngüsel davranışı şöyle işler: yeni bir sekme eklendiğinde,
mevcut bir sekme silindiğinde veya pencere yeniden boyutlandığında
paint tekrar çağrılır. `measured_tab_width` güncellenir ve sekmeler
güncel genişlikle yeniden render olur. Paint sırasında oluşan bu
...
(`system_window_tabs.rs:420`). Böylece sekme sayısı sıfır olduğunda
bile bölme işlemi güvenli kalır. Port hedefinde aynı geri besleme
döngüsü kurulmalıdır. Aksi halde sekme genişliği ya `0px` çıkar ya da
hiç güncellenmeden statik kalır.

**Sekme ölçüleri, close ayarı ve drop işaretleri** konuları
`system_window_tabs.rs:153-276` aralığında birlikte ele alınır. Burada
gözlenmesi gereken altı detay vardır:
...
Alt sağdaki plus bölgesi tek başına bir action değildir. Görsel olarak
da kendine ait bir yapısı vardır: `.h_full()` ile dikeyde tüm alana
yayılır, `DynamicSpacing::Base06.rems(cx)` ile yatay padding alır, üst
ve sol kenarına border çizilir, içine muted small bir plus ikonu
yerleştirilir (`system_window_tabs.rs:473-492`). Click akışı, yukarıda
anlatıldığı gibi `zed_actions::OpenRecent { create_new_window: true }`
action'ını dispatch eder. Bağımsız bir uygulamada genellikle aynı
görsel alan korunur; action ise ürünün kendi yeni pencere veya
workspace akışına yönlendirilir.
...
- `PlatformTitleBar::init(cx)` çağrısı tamamen kaldırılmaz. Bunun
  ...
  child'ı feature flag ile kapatılır. Böylece native tab desteğini
  ileride geri açmak kolay olur.
...
- `SystemWindowTabController::init(cx)` çağrısı GPUI `App` init
  ...
  fonksiyonu tekrar çağırır ve controller state'ini temiz biçimde
  yeniden başlatır. Manuel olarak tekrar tetiklemek gerekmez.
...
- Sekme kapatma ve yeni pencere action'ları uygulamanın lifecycle
  modeline doğrudan bağlanır. Bunlar boş bırakılırsa native tab yüzeyi
  görünür, ama çalışmaz durumda kalır.
