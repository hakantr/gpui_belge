...
Proje adı, `effective_active_worktree` ile seçilen çalışma ağacının kök adından üretilir. Çok uzun adlar başlığı taşırmasın diye bir üst sınıra kadar kısaltılır:
...
- **Çalışma ağacı seçici**: birden çok çalışma ağacı varsa, bağlı çalışma ağacı adıyla bir açılır panel tetikleyici.
- **Dal adı**: aktif dal bir açılır panel tetikleyicide gösterilir; `MAX_BRANCH_NAME_LENGTH` (40) sınırına kadar kısaltılır. Detached HEAD durumunda dal adı yerine kısa commit SHA'sı (`MAX_SHORT_SHA_LENGTH` = 8 karakter) veya port edilen arayüzde `"Dal Oluştur"` metniyle sunulan dal oluşturma eylemi gösterilir.
...
Dal tetikleyicisine bağlı `SwitchBranch` eylemi, dal değiştirme akışını başlatır. Dal adı yalnız `show_branch_name` ayarı açıkken metin olarak görünür; durum ikonu ise `show_branch_status_icon`'a bağlıdır. Böylece kullanıcı yalnız ikon, yalnız ad veya ikisini birden görmeyi seçebilir.
...
Port hedefinde git göstergesi tamamen opsiyoneldir. Bir git arayüzün varsa aynı kalıbı kurarsın: aktif dalı kısaltarak göster, tıklanınca dal değiştirme yüzeyini aç, opsiyonel olarak bir durum ikonu ekle. Detached HEAD gibi kenar durumları için (dal yok, yalnız SHA var) bir yedek metin hazırlamayı unutmazsın.
...
- **Bağlantı kopmuşsa**, devre dışı bir `"Bağlantı Koptu"` butonu gösterilir.
...
Davranış şudur: `TrustedWorktrees::has_restricted_worktrees` doğruysa, başlığa uyarı renginde bir `"Kısıtlı Mod"` butonu eklenir. Buton bir uyarı ikonu taşır ve ipucunda `ToggleWorktreeSecurity` eylemine işaret eder: "projeyi güvenilir işaretle ve tüm özellikleri aç". Tıklanınca çalışma alanının güven/güvenlik modalı açılır. Kısıtlı çalışma ağacı yoksa fonksiyon `None` döner ve hiçbir şey çizilmez.
