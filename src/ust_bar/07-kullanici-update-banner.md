...
**Bağlantı durumu:** `TitleBar::new(...)`, `banner` alanını `None` değeriyle kurar ve çalışma alanında `OnboardingBanner::new(...)` çağrısı bulunmaz. Duyuru bandı altyapısı kullanılabilir durumdadır, ancak başlıkta bir bant çizilmez. Render yolu `show_onboarding_banner` ayarının açık ve `banner` alanının `Some` olmasını birlikte gerektirir.
