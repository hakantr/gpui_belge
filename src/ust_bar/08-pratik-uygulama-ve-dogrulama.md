...
Menüleri ayrı bir satıra taşıma ihtiyacı varsa, [3. bölümdeki](03-titlebar-entity-ve-render.md#4-i̇ki-render-modu-menü-içeride-mi-ayrı-satırda-mı) iki render modu uygulanır: Menü modunda platform kabuğuna yalnızca menü teslim edilir, ürün içeriği ise aynı renk ve yükseklikte ikinci bir satırda çizilir.
...
- **Çocuk elementleri yalnızca bir kez vermek:** `PlatformTitleBar`, çocuk listesini render sırasında tüketir. Ürün başlığı her render geçişinde `set_children(...)` çağırmazsa içerik ikinci karede kaybolur ve başlık boşalır.
...
- **Görünürlük ayarlarını devre dışı bırakmak:** Render geçişini ayardan bağımsız yazıp her parçayı her zaman çizmek, kullanıcıya sade bir başlık sunma imkanını sınırlar. Her parça bir görünürlük bayrağına bağlanmalıdır.
...
- **İşbirliği/plan/duyuru bandı gibi Zed'e özgü parçaları gereksiz taşımak:** Bu arayüzler Zed'in ürün altyapısına bağlıdır; karşılığı olmayan bir uygulamada hiç kurulmamalıdır.

## 4. Davranış doğrulama listesi

Ürün başlığı entegrasyonu tamamlandığında aşağıdaki karar noktaları tek tek doğrulanmalıdır:
...
- İşbirliği/duyuru bandı gibi isteğe bağlı parçalar, karşılığı olmayan bir uygulamada hiç kurulmamış mı?

Bu liste, ürün başlığının davranış paritesini gözden geçirmek için yeterlidir. API isimleri ve davranışlar bu rehberde anlatıldığı gibidir; bir API'nin güncel tam imzasını teyit etmek istendiğinde Zed kaynağına başvurulması yeterlidir.
