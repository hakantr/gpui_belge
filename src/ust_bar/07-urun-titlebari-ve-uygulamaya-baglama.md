...
`PlatformTitleBar::is_multi_workspace_enabled(cx)` fonksiyonu Zed'de ilginç biçimde `DisableAiSettings` ayarı üzerinden değer üretir. İsim ürün açısından kafa karıştırıcıdır; davranış ise aslında bir feature flag gibi çalışır. Port hedefinde bu kontrolün `AppSettings::multi_workspace` veya `ShellSettings::sidebar_enabled` gibi doğrudan anlaşılır bir ayarla değiştirilmesi daha iyi olur.
