...
Bu bölüm ürün başlığının çekirdeğini açıklar: `TitleBar` entity'sinin nasıl kurulduğu, `init` ile uygulamaya nasıl bağlandığı ve her render geçişinde iki ayrı moddan birinin neden seçildiği ele alınmaktadır. Sonraki bölümlerdeki tek tek göstergeler bu çekirdeğin üzerine oturur.
...
Aynı blokta birkaç eylem `Workspace` içine bağlanır: Her platformda `UseClassicLayout` ve `UseAgenticLayout` (klasik ve yapay zekâ panel düzeni arasında geçiş) ile `SimulateUpdateAvailable` (güncelleme yüzeyini test etmek amacıyla); yalnızca macOS dışında ayrıca `OpenApplicationMenu`, `ActivateMenuLeft`, `ActivateMenuRight` (uygulama menüsünü açma ve menüler arası gezinme). Bu eylemler `Workspace` üzerinden gelir, başlık öğesini bulur ve ilgili `TitleBar` metoduna ya da düzen ayarına yönlendirir.

Port hedefinde de benzer bir kalıp kurulmalıdır: Uygulama başlangıcında platform kabuğu hazırlanır, ardından her pencere/çalışma alanı için bir ürün başlık entity'si üretilerek pencereye bağlanır.
...
`new` fonksiyonu, ürün başlığının ihtiyaç duyduğu tüm durum kaynaklarını toplar ve değişikliklerini izlemeye başlar:
...
2. **Katılımcı listesi**: Aktif çağrıdaki proje paylaşımı katılımcıları.
3. **İlk karşılama duyuru bandı**: Yalnız `show_onboarding_banner` ayarı açık ve `banner` alanı `Some` ise. (Güncel HEAD'de `banner` her zaman `None` olduğu için pratikte çizilmez; ayrıntılar [7. bölümde](07-kullanici-update-banner.md) ele alınmaktadır.)
4. **Sağ grup** (`h_flex`): İşbirliği kontrolleri, bağlantı durumu, güncelleme bildirimi, oturum açma butonu, oturum açılırken titreşen durum etiketi ve kullanıcı menüsü. Bu grup da sol grup gibi sol fare basma olayında yayılımı durdurur (aynı sürükleme çakışmasını önlemek için). Port hedefinde bu görünür metinler `"Oturum Aç"` ve `"Oturum açılıyor…"` gibi Türkçe yazılır.

Hangi parçanın görüneceği büyük ölçüde `TitleBarSettings` alanlarına bağlıdır (`show_project_items`, `show_branch_name`, `show_sign_in`, `show_user_menu` …). Ayarlar [4. bölümde](04-ayarlar-ve-uygulama-menusu.md) ayrıntılı olarak ele alınır.

## 4. İki render modu: menü içeride mi, ayrı satırda mı?

Render işleminin sonunda `show_menus` değeri iki ayrı yerleşim seçer. Bu, ürün başlığında dikkat isteyen kararlardan biridir.
...
Bu mod ayrımının pratik sonucu: Port hedefinde "menü açık mı?" kararı yalnız bir görünürlük bayrağı değildir; başlığın **kaç satır** olacağını da belirler. Menü ayrı satıra taşındığında ikinci satırın rengini ve yüksekliğini platform kabuğuyla eşitlemek gerekir, aksi halde iki katman görsel olarak kopuk görünür.
