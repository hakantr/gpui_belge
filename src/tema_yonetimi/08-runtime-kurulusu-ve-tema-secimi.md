...
`Theme` ve `IconTheme`'i doğrudan global yapmazsın; bunun yerine bir newtype wrapper kullanılır. Alanlar private'tır. Dışarıdan erişim `theme()`/`icon_theme()` ve update metotları üzerinden yapılır.
...
1. **Paritede kalmak** — trait'i Zed'deki gibi tek metotlu tutarsın; icon tema'ya `GlobalTheme::icon_theme(cx)` veya bağımsız bir `IconActiveTheme` trait'i üzerinden erişilir:
