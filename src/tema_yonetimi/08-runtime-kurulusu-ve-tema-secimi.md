...
Zed kaynak kodunda başlangıç teması için `DEFAULT_DARK_THEME` sabiti kullanılır ve güncel değeri `"One Dark"` olur. Ayna uygulamada sabitin adı korunabilir, ancak değer ürünün kendi yedek temasına işaret etmelidir; örneğin `pub const VARSAYILAN_KOYU_TEMA: &str = "Kvs Varsayılan Koyu";`. Önemli olan, `init` sonunda aktif tema seçiminin kayıtta gerçekten bulunan bir isimle yapılmasıdır. Sabit ile yedek tema adını ayrı ayrı elle yazarsan sonraki isim değişikliklerinde init kırılır.
...
- Tema bulunamazsa `ThemeNotFoundError` döner — çağıran bunu loglayabilir veya UI'da gösterebilir (toast: "Tema bulunamadı: X").
