...
5. **`SharedString` case sensitivity**: "Kvs Default" ile "kvs default" iki ayrı key olarak değerlendirilir.
...
| ---------- | ---------- | ------------- |
| `install_or_update_theme(cx, theme)` | `has_global`'a göre `set_global` veya `update_theme` çağırır | `set_theme` adı `theme_settings::settings::set_theme` ile çakışır — namespace karışıklığı bug çıkarır |
...
- `cx.window_appearance()` sorgusu yaparsın.
