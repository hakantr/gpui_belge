...
Üretilen temalar önce registry'ye ve global state'e yerleştirilir. Ardından sistem görünümü izlenir. Tema değiştiğinde pencereler yenilenir. Bu bölüm bu akışı sırayla anlatır: tema nerede tutulur, aktif tema nasıl seçilir ve UI yeni renkleri nasıl görür?

![Tema Runtime Akışı](assets/theme-runtime-akisi.svg)
...
`HashMap` sırasızdır; `sort()` deterministik bir liste sunar. UI'da tema seçici dropdown'u alfabetik görünür. Picker veya selector adın yanında appearance da gösterecekse `list()` çağrısı tercih edersin.
...
| Konu | Grup | API | Not |
|---|---|---|---|
| `GlobalTheme` | Metotlar | `icon_theme`, `new`, `update_icon_theme` | Builder, sorgu veya runtime çağrıları; ayrıntı bu konu anlatımındaki kullanım bağlamıyla okunur. |
...
`Theme` ve `IconTheme` doğrudan global yapılmaz; bunun yerine bir newtype wrapper kullanılır (Bölüm III/Konu 10 kuralı). Alanlar private'tır. Dışarıdan erişim `theme()`/`icon_theme()` ve update metotları üzerinden yaparsın.
...
**Önemli sözleşme notu:** Zed'de `ActiveTheme` trait'i **yalnızca** `theme()` metoduna sahiptir; `icon_theme()` trait'in parçası **değildir**. Aktif icon tema'ya erişim `GlobalTheme::icon_theme(cx)` üzerinden yaparsın. Zed paritesinde `cx.icon_theme()` doğrudan çalışmaz.
...
Rehberin örnekleri 1. seçeneği varsayar; aktif icon tema için `GlobalTheme::icon_theme(cx)` çağırırsın. 2. seçenek uygulandığında ilgili çağrılar `cx.icon_theme()` olarak kısaltılabilir.
...
UI bileşeni tema değişimini takip etmek istediğinde aşağıdaki desen kullanırsın:
...
| Konu | Üyeler | Not |
|---|---|---|
| `SystemAppearance` | `deref`, `Target` | Trait impl üzerinden gelen public üyelerdir; çoğu dönüşüm, render, builder veya standart trait köprüsüdür. |
...
| Konu | Grup | API | Not |
|---|---|---|---|
| `SystemAppearance` | Metotlar | `global_mut` | Builder, sorgu veya runtime çağrıları; ayrıntı bu konu anlatımındaki kullanım bağlamıyla okunur. |
| `SystemAppearance` | Alanlar | `0` | Public veri alanları; runtime, stil veya ayar sözleşmesinin taşınan parçalarıdır. |
...
> **Neden `Appearance` doğrudan global yapılmaz?** `Appearance` enum'u farklı anlamlarda da kullanırsın: tema'nın nominal modu ve JSON deserialize hedefi gibi. Global anahtarının **sisteme özgü** kalması önemlidir. `SystemAppearance` yalnızca "OS şu an ne söylüyor?" sorusunu cevaplar.
...
- `cx.window_appearance()` (Bölüm III/Konu 8) sorgusu yaparsın.
- `WindowAppearance` dört variant'tan birini döndürür; `Vibrant*` macOS'a özgüdür ama tema seçiminde Light/Dark ile aynı kategoride ele alırsın.
- Match ifadesi dört variant'ı iki kategoride birleştirir.
- `set_global` ile global kurarsın.
...
| Konu | Grup | API | Not |
|---|---|---|---|
| `LoadThemes` | Varyantlar | `All`, `JustBase` | Enum seçim değerleri; davranış farkı ilgili konu anlatımında verilir. |
...
Zed kaynak kodunda başlangıç teması için `DEFAULT_DARK_THEME` sabiti kullanılır ve güncel değeri `"One Dark"` olur. Mirror uygulamada sabitin adı korunabilir, ancak değer ürünün kendi fallback temasına işaret etmelidir; örneğin `pub const DEFAULT_DARK_THEME: &str = "Kvs Default Dark";`. Önemli olan, `init` sonunda aktif tema seçiminin registry'de gerçekten bulunan bir isimle yapılmasıdır. Sabit ile fallback tema adını ayrı ayrı elle yazarsan sonraki isim değişikliklerinde init kırılır.

Zed kaynak kodunda bu invariant fail-fast açılır; rehber örneğinde ise aynı sözleşme `?` ile dışarı taşınır. Az önce UI fallback temaları insert edildi; registry de default icon tema'yı kurdu. Eksik olmaları beklenmez. Eksiklerse bu programatik bir hatadır (typo veya init bug'ı) ve uygulama başlangıcında görünür olmalıdır.

> Alternatif olarak `SystemAppearance` baz alınarak başlangıç teması seçebilirsin:
> ```rust
> let default_name = match SystemAppearance::global(cx).0 {
>     Appearance::Dark => "Kvs Default Dark",
>     Appearance::Light => "Kvs Default Light",
> };
> let default = registry.get(default_name)?;
> ```
...
5. **Default tema seçiminde fallback olmadan**: `registry.get("Yok")` hatası kullanıcıya veya init log'una taşınmalıdır. Yalnızca insert edilmiş temaların seçmen gerekir.
...
pub fn init(themes_to_load: LoadThemes, cx: &mut App) {
    ...
    let default = registry.get("Kvs Default Dark")?;
    let default_icon = registry.default_icon_theme()?;
...
`init-or-update` pattern (Konu 34) burada da geçerlidir. İlk çağrıda global kurulur, sonraki çağrılarda ise içeride mutate edilir. `observe_global::<GlobalTheme>` observer'ları varsa tetiklersin.
...
3. **Reload sonrası `aktif_ad` lookup'ının başarısız olması**: Tema dosyasından o isim silinmiş olabilir. `get(&aktif_ad).is_err()` durumunda default fallback'e düşülür:
   ...
   match registry.get(&aktif_ad) {
       ...
       Err(_) => {
           ...
           let fallback = registry.get("Kvs Default Dark")?;
