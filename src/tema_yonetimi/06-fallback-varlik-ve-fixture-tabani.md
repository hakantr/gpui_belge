...
Yedek temalar çalışma zamanı için bir **güvenlik ağıdır**. Kullanıcı temasının yüklenmesi başarısız olsa bile uygulama açılabilmelidir. Bu yüzden tema kaydında her zaman en az **iki temayı** hazır bulundurursun: biri açık, biri koyu.
...
- Lightness değerlerini tersine çevirirsin: koyuda 0.12 olan arka plan, açıkta 0.98 olur.
- Saturation'ı çoğunlukla aynı tutarsın; kullanıcı baktığında "aynı tema, ters mod" hissini almalıdır.
...
6. **Light temasını dark'tan otomatik türetmek**: `l = 1.0 - dark_l` gibi formüller **çalışmaz** — gözün light ve dark algısı doğrusal değildir. Light temayı ayrı bir tasarım kararı olarak yazarsın.
...
- `ColorScales` aynalıyorsan `default_color_scales()` çıktısını aileye bağlarsın; aynalamıyorsan bu alan için yerel bir kararı açıkça verirsin.
...
Tüketici bileşen `list_font_families` çağrısını her render'da yapmamalıdır. Font seçici gibi bir yüzey açıldığında listeyi alırsın, UI durumuna koyarsın ve tema değişiminden bağımsız olarak kullanırsın. Tema renkleri değiştiğinde font ailesi listesi değişmez; sistem fontları değiştiyse önbellek geçersiz kılma için ayrı bir uygulama kararı gerekir.
...
2. **`RustEmbed` derleme süresini şişirir**: Her tema dosyası binary'ye gömülür. 100 MB'lık bir tema klasörü 100 MB'lık binary'ye dönüşür. Ayıklama için include filtreleri kullanabilirsin:
   ...
   ```
3. **Async yüklemede `cx` lifetime'ı**: `cx.spawn` içinde `AsyncApp` bulunur; eşzamanlı bağlama geçmek için `cx.update(|cx| ...)` kullanırsın.
...
| Tip | Lisans | Sözleşme |
| ----- | -------- | ---------- |
| Kod bağımlılığı | Apache-2.0 | Doğrudan bağımlılık olarak kullanırsın |
| Kod bağımlılığı | Apache-2.0 | Doğrudan bağımlılık |
| Kod bağımlılığı | Apache-2.0 | Doğrudan bağımlılık |
| Kod referansı | GPL-3.0-or-later | **Aynalarsın, kopyalamazsın** |
| Kod referansı | GPL-3.0-or-later | **Yalnızca referans için okursun, bağımlılık olarak almazsın** |
| Veri örneği | Tema-özel (MIT/Apache/GPL) | **Lisans dosyasıyla birlikte kopyalarsın** |
| Tasarım verisi | GPL-3.0-or-later | **Kopyalamazsın; bağımsız palet seçersin** |
...
Dosya adı `LICENSE` olduğunda içeriği tema ile birlikte saklarsın. MIT, Apache ve BSD gibi uyumlu lisansları paket içine alabilirsin. GPL veya lisansı belirsiz tema dosyalarını ise pakete dahil etmezsin. Her paketlenen tema için kaynak repo, yol, lisans ve telif bilgisi atıf dosyasında yer almalıdır.
...
- `from_content` çağrısı ile **tam akışı** test edersin: ayrıştırma + refinement + `Theme` yapısı.
...
6. **Örnek veri dosyasının yerinde değiştirilmesi**: Test örnek verisine yama uyguladığında, testler kendi verisini yazıp doğrulamış olur. Örnek veri dosyaları **salt okunur** kabul edilmelidir; sentetik kenar durumlarını ayrı dosyalarda veya satır içi string'lerde tutarsın.
