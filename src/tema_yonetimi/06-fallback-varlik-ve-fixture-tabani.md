...
Yedek temalar çalışma zamanı için bir **güvenlik ağıdır**. Kullanıcı temasının yüklenmesi başarısız olsa bile uygulama açılabilmelidir. Bu yüzden tema kaydında her zaman en az **iki tema** hazır bulundurulur: biri açık, biri koyu.
...
**Açık eşleniği** (`durum_renkleri_acik()`): Aynı çapa renkleri kullanılır. Yalnızca **lightness değerleri biraz koyulaştırılır**; böylece açık arka plan üzerinde okunaklılık korunur. Arka plan ve kenarlık opacity'leri de açık zemine göre biraz daha düşük tutulur.
...
| Boyut | Koyu | Açık |
| ------- | ------ | ------- |
| Ön plan lightness | 0.55–0.60 | 0.40–0.50 (koyu arka plana karşı kontrast için) |
| Arka plan opacity | 0.20 | 0.15 (açık yüzeyde aşırı dolgu olmaması için) |
| Border opacity | 0.50 | 0.40 |
| Saturation | aynı | aynı |
...
- Vurgu için koyudaki hue (örn. 210°) açıkta biraz **hacimli ve koyu** konumlanır (koyudaki l=0.60 yerine açıkta l=0.50); açık arka plan üzerinde okunaklılık için.
...
2. **`unsafe { std::mem::zeroed() }` kullanımı**: Sonuç yine sıfır `Hsla` olur. Şablon kodda görüldüğünde silinmeli, gerçek kodda hiç kullanılmamalıdır.
...
4. **`palette` sürümünün sabitlenmesi**: Aynı `hsla(0.583, 0.10, 0.12)` ifadesi, farklı `palette` major sürümlerinde **ufak miktarda farklı sRGB** üretebilir. Cargo.lock dosyası ile kullanılan sürümün sabitlenmesi yerinde olur.
5. **Zed'in `default_colors` HSL değerlerini birebir kopyalamak**: GPL-3 ihlali demektir. Bağımsız çapa değerleri seçilmelidir.
6. **Light temasını dark'tan otomatik türetmek**: `l = 1.0 - dark_l` gibi formüller çalışmaz — gözün light and dark algısı doğrusal değildir. Light tema ayrı bir tasarım kararı olarak yazılır.
7. **Boş syntax yedeği**: Yedek olarak boş syntax kabul edilir; ancak UI'da kod gösteriliyorsa en azından temel kategorilerin (`comment`, `string`, `keyword`, `number`, `function`) doldurulması gerekir. `Theme.styles.syntax` alanı `Arc<SyntaxTheme>` tipi beklediği için, içerik boş olsa bile `Arc::new(...)` sarması zorunludur.
...
Ayna uygulamada bu fonksiyon birebir kopyalanmaz; lisans ve ürün kimliği nedeniyle kendi adıyla, örneğin `kvs_default_themes()` biçiminde yazılır. Ancak davranış sözleşmesi aynıdır:
...
- `ColorScales` aynalanıyorsa `default_color_scales()` çıktısı aileye bağlanır; aynalanmıyorsak bu alan için yerel bir karar açıkça verilir.
...
Bu iki yardımcının yeri yedek tema dosyasıdır, ama çağrıldıkları ana akış refinement ve tema üretimidir. Yerleşik yedek temayı yazarken alanlar zaten tam doldurulur; yardımcılar asıl olarak kullanıcı JSON'u eksik alan verdiğinde anlam kazanır.
...
| `FontFamilyCache::global(cx)` | Global önbelleği `Arc<FontFamilyCache>` olarak döndürür. | Ayarlar UI'ı veya font seçici önbelleğe erişileceğinde. |
...
| `FontFamilyCache::prefetch(cx).await` | Font listesini async app üzerinden arka planda ısıtır. | Ayarlar ekranı açılmadan önce hazırlık yapılmak istendiğinde. |

Tüketici bileşen `list_font_families` çağrısını her render'da yapmamalıdır. Font seçici gibi bir yüzey açıldığında liste alınır, UI durumuna konulur ve tema değişiminden bağımsız olarak kullanılır. Tema renkleri değiştiğinde font ailesi listesi değişmez; sistem fontları değiştiyse önbellek geçersiz kılma için ayrı bir uygulama kararı gerekir.

---

## 26. Yerleşik tema paketleme ve `AssetSource`

Yerleşik temalar, uygulama ile **birlikte dağıtılan** JSON tema dosyalarıdır. Bunlar paketlenirken üç stratejiden biri tercih edilebilir; seçim ihtiyaca göre yapılır.
...
- `tema_icerigi.appearance` değerine göre uygun taban seçilir.
- Bir tema dosyasından hata gelse bile diğerlerinin yüklenmeye devam etmesi isteniyorsa `try_into`/`continue` deseni kullanılabilir:
...
GPUI'nin kendi varlık sistemi kullanılacaksa, özellikle SVG ve ikonlarla tutarlı bir varlık hattı hedefleniyorsa, bu strateji seçilebilir.
...
GPUI'nin `cx.asset_source()` API'sı ile tema dosyaları doğrudan aynı varlık kaynağından okunabilir. `Resource::Embedded(...)` de bu kaynağı kullanan GPUI tüketicileriyle aynı paketleme düzenini paylaşır:
...
- İkon, SVG and font ile tutarlı tek bir API.
- Varlık önbelleği ve yükleme davranışının GPUI tarafından yönetilmesi.

**Dezavantajlar:**

- `AssetSource` impl şablon kodu.
- GPUI sürüm değişimlerinde trait imzasının kayma ihtimali.
...
1. **`temalar_dizini` çalışma dizini bağımlılığı**: Disk yüklemesinde `assets/themes` göreli bir yoldur; binary nereden çalıştırılırsa yola göre çözülür. **Mutlak yol** üretilmesi yerinde olur:
   ...
   ```
2. **`RustEmbed` derleme süresini şişirir**: Her tema dosyası binary'ye gömülür. 100 MB'lık bir tema klasörü 100 MB'lık binary'ye dönüşür. Ayıklama için include filtreleri kullanılabilir:
   ```toml
   #[derive(RustEmbed)]
   #[folder = "assets/"]
   #[include = "themes/**/*.json"]
...
5. **Taban seçiminin görünür olması**: Dark bir temanın tabanı light olarak verilirse, eksik alanlar açık tema değerlerinden gelir ve görsel olarak uyumsuz bir tema ortaya çıkar. Tabanın `appearance` değerine göre seçilmesi gerekir.
...
7. **Sıcak yeniden yüklemenin üretimde açık kalması**: Dosya izleyici CPU/IO maliyetinin yanı sıra güvenlik açısından da risk taşır. `cfg(debug_assertions)` ile sınırlandırılması tercih edilir.
...
Hangi yapı seçilirse seçilsin, `RustEmbed`/`AssetSource` filtrelerinin buna göre güncellenmesi gerekir. Dosya adı `LICENSE` olduğunda içeriği tema ile birlikte saklanır. MIT, Apache ve BSD gibi uyumlu lisanslar paket içine alınabilir. GPL veya lisansı belirsiz tema dosyaları ise pakete dahil edilmez. Her paketlenen tema için kaynak repo, yol, lisans ve telif bilgisi atıf dosyasında yer almalıdır.
...
3. **Atıf README'sinin güncellenmemesi**: Yeni bir tema eklendikten sonra atıf eklenmediğinde, telif sahibi gösterimi şartı ihlal edilmiş olur.
...
- `any::<Hsla>()` alpha dahil tüm kanalları `0.0..=1.0` aralığında üretir; ayrıştırma/refinement yuvarlama ve alpha davranış testlerinde kullanılır.
...
2. **Örnek veri lisansının birlikte taşınması**: Dosya kopyalanırken `*.LICENSE_*` dosyalarının da taşınması gerekir; örnek veri JSON'u ile lisans dosyası bir arada tutulur.
3. **Test'lerin `init` çağrısı**: `kvs_tema::init(cx)` çağrısı her test başında elle yapılır; otomatik bir kurulum mekanizması yoktur. Çağrı `TestAppContext::update` callback'i içinde yazılır.
