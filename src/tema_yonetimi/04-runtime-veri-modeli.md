...
çağrısı `&Arc<Theme>` döndürür. Bu yüzden yaygın kullanımda clone maliyeti yalnızca `Arc` refcount artışıdır. Doğrudan `Theme::clone()` çağırmak çoğu zaman gerekmez; `GlobalTheme.theme` zaten `Arc<Theme>` üzerinden taşınır.
...
> **Bu tablo yaklaşık sayılar verir.** Tam liste için `crates/theme/src/styles/colors.rs` runtime alanları ve `crates/settings_content/src/theme.rs` Content alanları esas alınır.
...
3. **Yeni grup eklendiğinde Konu 13 tablosunu güncellememek**: Yeni bir semantik grup eklendiyse rehberin bu bölümündeki tabloya satır eklenmesi gerekir; aksi halde dokümantasyon kodun gerisinde kalır.
...
`StatusColors` da `ThemeColors` gibi her alanı `Hsla` olarak tutar. Eksik alanlar runtime struct'ında değil, refinement katmanında ele alınır. `StatusColorsRefinement` her alanı otomatik olarak `Option<Hsla>` haline getirir.
...
"Player" terimi Zed'in collaboration sisteminden gelir. Aynı dosyada eş zamanlı düzenleme yapan her kullanıcıya ayrı bir **cursor**, **selection** ve **background** rengi atanır. Bu yapı single-player uygulamalarda da işe yarar; örneğin multi-cursor görünümlerinde kullanılabilir.
...
- Boş liste durumu sözleşmeyle dışarıda bırakılır. Yine de defansif kod gerektiren yerlerde `Default::default()` ile fallback kurulmalıdır; aksi halde sıfır eleman panic riski oluşturur.
...
6. **`AccentColors` iç tipini `Vec<Hsla>` yapmak**: Sözleşme `Arc<[Hsla]>` üzerinedir. `Vec` yazıldığında baseline'dan yapılan klon her tema varyantında yeni bir allocation üretir; bu durum `Arc<[T]>`'nin cheap-clone garantisini bozar.
...
Bu üç tip, tema'nın **paketleme ve uzantı** tarafını taşır. `ThemeFamily` bir paket içindeki birden fazla varyantı tek çatı altında toplar. `SyntaxTheme` syntax token'larının ayrı sözleşmesini taşır. `IconTheme` ise icon tema sözleşmesinin runtime modelini kurar.
...
JSON tarafında bu yapı bir object olarak yer alır ve sırası `IndexMap` ile korunur. Rust runtime'a `Vec<(String, HighlightStyle)>` listesi olarak iletilir. `SyntaxTheme::new` bu listeyi tüketir ve iki private alana (`highlights` ve `capture_name_map`) ayırır.
...
Editor entegrasyonu Bölüm X ve XI'de ele alınır. `SyntaxTheme::merge(base, override)` helper'ı override'ları base'in üstüne uygular ve yeni bir `Arc` döndürür. Tema override'ları (Bölüm IX/Konu 39) bu helper'ı çağırır.
...
UI temasından farklı olarak burada **refinement katmanı yoktur**. Yani `Refineable` türevli alan-bazlı tema override hattı icon tema için çalışmaz. Buna rağmen Zed'in yükleme ve lookup davranışı "tam replacement" da değildir. `ThemeRegistry::load_icon_theme`, kullanıcı temasının `file_stems`, `file_suffixes` ve `named_directory_icons` haritalarını default icon theme'in üstüne genişletir. `directory_icons`, `chevron_icons` ve `file_icons` alanları runtime objesine kullanıcının verdiği biçimde girer. Lookup sırasında ise eksik dosya tipi, klasör ve chevron path'leri aktif temadan default icon theme'e düşebilir. Mirror tarafta bu iki aşama ayrı düşünülmelidir: schema/refinement yoktur, ama registry yükleme ve UI lookup fallback'i vardır.
