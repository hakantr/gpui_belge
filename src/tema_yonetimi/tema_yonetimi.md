...
Bu rehber **mimari, sözleşme ve kod** tarafına odaklanır; tema sisteminin
public API yüzeyini, JSON sözleşmesini ve GPUI runtime davranışını aynı
yerde toplar.

Her konu kendi başına okunabilir; kullanılan tipi, hangi modülden geldiğini,
neyi kabul ettiğini, runtime davranışını ve yaygın tuzakları tek yerde
toplar.
...
3. [Lisans sınırları](#3-lisans-sınırları)
...
34. [JSON schema ve fixture sözleşmesi](#34-json-schema-ve-fixture-sözleşmesi)
35. [Tema asset lisans sınırları](#35-tema-asset-lisans-sınırları)
...
uyumluluk alanı vardır; onlar Bölüm IV/Konu 18'de ayrıca belirtilir.
...
- `palette` sürümü Zed'in kullandığı sürümle uyumlu olmalı (Bölüm I/Konu 5).
...
`apply_theme_color_defaults` tarafındaki ek türetme kuralları Konu 43.6'da
ayrıca anlatılır.
...
temalarının nasıl paketlendiği, fixture/schema sözleşmesi ve lisans
gereksinimleri.
...
### 34. JSON schema ve fixture sözleşmesi
...
### 35. Tema asset lisans sınırları
...
Son bölüm, önceki sekiz bölümdeki API ve kullanım kalıplarını pratik
başlıklarda toplar. Yaygın tuzaklar ilgili bölümlere işaret eder; ileri
öğeler ise günlük tema yüklemesi için zorunlu olmayan public yüzeyleri
tek yerde kataloglar.

> **Kullanım sözleşmesi:** Bu bölüm referans niteliğinde. Önceki
> bölümleri **bilmek koşulu değil**; ayrıntı gereken her maddede ilgili
> konu numarasına dönülebilir.
...
> **Mirror disiplini hatırlatması:** Bu öğelerden hangilerinin public API'ye
> dahil edildiğini açık tut. "Şimdilik gerek yok" diye atlanan yüzeyler
> ileride tüketici sözleşmesine eklenirken ayrıca tasarlanmalıdır.
...
#### 43.9 Son public yüzey — küçük ama atlanabilir parçalar
...
| `gpui::Rgba::try_from(&str)` desteklediği formatlar | `gpui/src/color.rs:162-256` | **Dört format**: `#rgb` (3 hane, çiftleme), `#rgba` (4 hane, çiftleme), `#rrggbb` (6 hane), `#rrggbbaa` (8 hane). `#` zorunlu; trim yapılır. Konu 20'de ayrıntılı |
...
| `ThemeSettingsContent.ui_density` JSON rename | `settings_content/src/theme.rs:166-167` | `#[serde(rename = "unstable.ui_density")]` — kullanıcı JSON'unda **`"unstable.ui_density"`** yazmalıdır; `"ui_density"` çalışmaz. Konu 43.3'te ayrıntılı |
