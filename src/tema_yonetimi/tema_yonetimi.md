...
Okuma sırası bilinçli olarak aşağıdaki gibi düzenlenmiştir. Önce kapsam ve lisans sınırı netleşir, sonra proje iskeleti kurarsın. GPUI tipleri ve runtime veri modeli anlaşıldıktan sonra JSON sözleşmesi, refinement, registry, ayar entegrasyonu ve UI tüketimi gelir. Son iki bölüm ise public API ile test ortamını toparlar ve pratik kontrol listesi sunar.
...
4. [Runtime veri modeli](04-runtime-veri-modeli.md) `Theme`, `ThemeColors`, `StatusColors`, player/accent renkleri, syntax tema ve icon tema modelini kurarsın.
...
9. [Ayarlar ve yoğunluk entegrasyonu](09-ayarlar-ve-yogunluk-entegrasyonu.md) Tema seçim ayarlarını, provider sınırını, font override davranışını ve `UiDensity` sözleşmesini kurarsın.
