...
| `markdown_preview_font_size` settings alanı | Markdown preview metin boyutunu ayrı seçer. Boş bırakıldığında `ThemeSettings::markdown_preview_font_size(cx)` ayar dosyasındaki `ui_font_size` değerine düşer; geçici UI zoom global'i bu fallback'i etkilemez. Provider trait'i değişmez. |
