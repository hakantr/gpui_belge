...
`Arc<str>` veya `&'static str` taşıyabilen, **ucuz klonlanan** değişmez bir string tipidir. Her render geçişi kare kare yeniden çalışır. Bu yüzden `String::clone()` her seferinde yeni bellek ayırma üretirse maliyet birikir. `SharedString::clone()` ise yalnızca `Arc` referans sayacını artırır; yeni bellek ayırma yapmaz.
...
- `Clone` — `Arc` referans sayacı; bellek ayırma yoktur.
...
- `Eq` ve `Hash`, `f32` taşıyan `fade_out` alanı için elle uygulanır (`f.to_be_bytes()` ile `u32`'ye çevrilir; `NaN` değerleri `0`'a düşer).
...
fn vurgulama_stili(icerik: &HighlightStyleContent) -> HighlightStyle {
    HighlightStyle {
        ...
        font_weight: icerik.font_weight.map(|agirlik| FontWeight(agirlik.0)),
        ..Default::default()
    }
}
```
