...
**Tüketici sözleşmesi:** UI bileşenleri tema değerlerine `cx.theme()` çağrısı üzerinden erişir. Bu çağrı `&Arc<Theme>` döndürür; klon ve bellek ayırma üretmeden okuma yapılır.
...
`ThemeStyles`, `ThemeColors`, `StatusColors`, `PlayerColors`, `AccentColors`, `SystemColors` ve `SyntaxTheme` çalışma zamanı veri modelinin parçalarıdır; UI tarafında çoğunlukla bu tipler doğrudan kurulmaz, aktif `Theme` üzerinden okunur. Bu ayrım önemlidir: bileşen kodu tema üreticisi değildir, tema tüketicisidir.
...
`ThemeColorField` ve `all_theme_colors(cx)` normal bileşen render'ı için değil, tema editörü, color picker, debug inspector ve snapshot ekranı için düşünülür. `ThemeColorField` `ThemeColors` alanlarının reflection alt kümesini isimlendirir; `all_theme_colors(cx)` aktif temadan bu alanları `(Hsla, SharedString)` listesi olarak verir. Bir buton arka planı çizileceği zaman `all_theme_colors` dolaşılmaz; doğrudan `theme.colors().element_background` okunur.
...
Zed'in `Theme::darken(color, light_amount, dark_amount)` yardımcısı, bir rengi appearance'a göre **lightness** azaltarak koyulaştırır. Light tema modunda `light_amount`, dark tema modunda `dark_amount` kullanılır. Sonuç `l = (l - amount).max(0.0)` ile alt sınırlanır. Aynı bileşenin iki temada da yeterli kontrastı koruması için iki ayrı miktar verilir.
...
`.when(kosul, |oge| ...)` koşullu bir fluent yardımcısıdır. Devre dışı durumda hover, active and click handler tamamen atlanır.
...
"Ghost" terimi, transparan arka planlı bir elementi tanımlar. Araç çubuğu ikon butonları gibi yüzeye yapışmış görünen bileşenlerde kullanılır.
...
`ghost_element_background` genellikle `hsla(0, 0, 0, 0)` (tamamen şeffaf) olarak tutulur. Hover durumunda `ghost_element_hover` devreye girer ve element görünür hale gelir. Bu alan opak bir yüzey rengi değil, şeffaf zemin üstüne serilen ince, düşük-alpha bir nötr katmandır; `elevated_surface_background` ise opak bir nötr basamaktır, o yüzden ikisi karıştırılmamalıdır.
...
**Genel kural:** Element'in **kendine ait bir kromu** varsa (border, görünür arka plan) → `element_*` seçilir. Yüzeye yapışmış, yalnızca hover/active'de görünen bir element ise → `ghost_element_*` tercih edilir.
...
Bırakma hedefi alanları, sürükleme işlemi sırasında kullanıcıya "buraya bırakılabilir" geri bildirimi vermek için kullanılır.
...
2. **`.hover` callback'inde tekrar `cx` üzerinden tema erişimi**:
   ...
   ```
3. **Hover ile active sıralamasının tersine yazılması**: `.active(...).hover(...)` yazılsa bile davranış aynıdır; ancak okunabilirlik için `hover → active` sıralaması idiomatik kabul edilir.
4. **`element_disabled` yerine `element_background.opacity(0.5)` tercih etmek**: İki seçenek farklı tasarım kararlarıdır. Tema yazarı devre dışı durum için özel bir renk vermiş olabilir; bu durumda `element_disabled` alanının kullanılması gerekir.
...
7. **Etkileşim durumlarının kontrast olmayan renklerle verilmesi**: `element_hover` ile `element_background` arasında yeterli lightness farkı bulunmadığında kullanıcı hover etkisini fark etmez. Tema testlerde bu farkın gözle doğrulanması yerinde olur.
