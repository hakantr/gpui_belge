...
`BoxShadow::blur_radius` yalnızca gölge primitive'inin yumuşatma yarıçapıdır. Bu alan, elementin arkasındaki sahneyi veya masaüstünü bulanıklaştıran bir `backdrop-filter` karşılığı değildir. GPUI `Style` yüzeyinde `background`, `border_color`, `corner_radii`, `box_shadow`, `text`, `opacity` ve yerleşim alanları bulunur; element düzeyinde `filter`, `backdrop_filter` veya benzeri bir blur/post-process alanı yoktur. Bu yüzden buton, sidebar ya da panel gibi tekil elementlerde cam efekti; yarı saydam renklerin pencere düzeyindeki `WindowBackgroundAppearance::Blurred` arka planı göstermesiyle elde edilir, elementin kendisine özel bir blur işlemiyle değil.
...
- `Rems(f32)`: Kök yazı tipi boyutuna göre ölçeklenir (Zed bünyesinde `theme.ui_font_size` değerine bağlıdır). `.text_sm()` veya `.gap_2()` gibi otomatik stil yardımcıları, genellikle arka planda Rems üzerinden mantıksal piksellere dönüşüm yapar. `Rems::ZERO` sıfır ölçüsünü sabit olarak verir; `Rems::from_pixels(length, window)` pencerenin anlık `rem_size()` değerine göre mantıksal pikseli rem ölçüsüne geri çevirir.
