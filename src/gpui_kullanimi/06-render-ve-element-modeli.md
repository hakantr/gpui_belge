...
**Crate kökü ve kapalı trait sınırları.** `gpui` crate kökü bazı düşük seviyeli sembolleri de dışa aktarır. `GPUI_MANIFEST_DIR`, `env!("CARGO_MANIFEST_DIR")` değerini taşıyan `#[doc(hidden)]` bir sabittir; asset veya test altyapısı crate'in manifest dizinine ihtiyaç duyduğunda kullanılır, uygulama verisi yolu veya kullanıcı ayarı yolu olarak kullanılmaz. `Sealed` ise GPUI'nın belirli trait implementasyonlarını yalnız crate içinde tutmak için kullandığı kapalı trait desenidir. Bir trait `Sealed` ile sınırlandığında uygulama kodu o trait'i yeni bir tipe uygulamaya çalışmak yerine GPUI'nın sağladığı element, input veya event yüzeylerinden ilerlemelidir.

| API | Alt özellikler | Kısa anlamı |
| :-- | :-- | :-- |
| `Sealed` | kapalı trait marker'ı | GPUI'nın bazı trait implementasyonlarını crate dışına kapatır; dış kod yeni implementasyon yazmak yerine hazır element/event yüzeyini kullanır. |
