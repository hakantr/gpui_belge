...
#### Yeni action yazarken kullandığın çekirdek yüzey (`crates/gpui/src/action.rs:117+`)
...
- **`zed::Unbind(SharedString)`** — `derive(Action)` ile tanımlıdır; yükü (`payload`) bir action adıdır. JSON biçimi şu şekildedir:
  ...
  Keymap ayrıştırıcısı bu nöbetçiyi gördüğünde aynı keystroke'a aynı action adıyla ileriden gelen tüm kısayolları **action bağlamından bağımsız olarak** iptal eder (`keymap.rs:124-128`'deki `is_unbind` kolu). Yani `editor::NewLine` için `enter` tuşunun tüm bağlamlarda unbind edilmesi gerektiğinde kullanırsın.
