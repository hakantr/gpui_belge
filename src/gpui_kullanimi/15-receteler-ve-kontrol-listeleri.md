...
Desteklenen Git değişkenleri seçili commit ve onun bağlı olduğu repository bağlamından üretilir. Bu bağlamda yalnız Git değişkenleri sağlarsın. `ZED_FILE`, `ZED_SELECTED_TEXT`, `ZED_WORKTREE_ROOT`, `ZED_MAIN_GIT_WORKTREE` gibi editör/worktree değişkenleri varsayılan değer taşımadıkça çözümlenmez.
...
Aşağıdaki liste, rehberde anlatılan kavramların hangi crate veya modülde yer aldığını tek bakışta verir. Yeni bir bileşen yazarken benzer örneğin nerede tanımlı olduğunu hızla bulmana yarar ve aranan konunun hızla bulunmasını sağlar.
