...
Benchmark platformu `bench_platform(...)` ile kurulur. Bu platform `BenchDispatcher` kullanarak ön plan ve arka plan işlerini gerçek zamanlı, çok iş parçacıklı bir kuyrukta yürütür; `#[gpui::bench]` tarafından üretilen platform mevcut platformun text system'ini kullandığı için metin ağırlıklı ölçümler gerçek metin şekillendirme ve glif rasterizasyon maliyetini içerir. `BenchAppContext::bench_iter(...)` genel iş yükü ölçümü, `BenchAppContext::bench_renderer(...)` ise bir `Entity<V: Render>` güncellemesinden sonra çizilen frame'leri ölçmek için kullanılır. `BenchReport` frame bütçesi, draw süresi, dirty-to-draw süresi ve invalidation sayısı gibi rapor değerlerini toplar. Ölçüm bitiminde önceki ölçümden kalan zamanlayıcıların sonraki benchmark'a taşınmaması için `BenchDispatcher::cancel_pending_timers()` kullanılabilir; dispatcher iç döngüsünü elle sürmek gereken düşük seviye koşullarda `run_ready_main_tasks()` ve tanı amacıyla `debug_state()` yüzeyi de vardır.
