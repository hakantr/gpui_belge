...
GPUI test yazımında takip edilen genel disiplinler şunlardır:

- `#[gpui::test]` makrosu ve `TestAppContext` yapısı tercih edilir.
- Pencere gereksinimlerinde test bağlamının (context) offscreen veya window yardımcı metotları tercih edilir.
- Asenkron timer işlemleri için `cx.background_executor().timer(duration).await` çağrısından yararlanılır.
- Arayüz (UI) action testlerinde keybinding ve action dispatch işlemleri doğrudan test edilir.
- Görsel test gereksinimlerinde `VisualTestContext` ve başsız (headless) renderer desteği kontrol edilmelidir.
- Element hata ayıklama sınırları için test-support altında `.debug_selector(...)` eklemesi yapılır.
- `gpui` `proptest` özelliği açıkken `Hsla` için `Arbitrary` uygulaması ve `Hsla::opaque_strategy()` yardımcısı bulunur. Renk veya kontrast özellik (property) testlerinde alfa değeri 1.0 olan rastgele renkler üretmek için bu yardımcıdan yararlanılır.

**Testlerde Kaçınılması Gereken Kalıplar.** Aşağıdaki desenler test sonuçlarını güvenilmez yapar:
...
- `cx.run_until_parked()` — tüm bekleyen future ve task tamamlanana kadar devam ettirilir.
...
`AnyWindowHandle::build_entity(cx, |window, cx| ...)`, elde yalnız tip silinmiş pencere handle'ı varken o pencere içinde yeni bir `Entity<V>` kurmayı sağlayan test yardımcısıdır. `VisualTestContext::deactivate_window()` aktif test penceresini blur eder ve ardından arka plan executor'ını park edene kadar sürer; focus kaybı, menü kapanması veya inactive window stili testlerinde doğrudan bu davranış doğrulanır.

**Pratik Kurallar.** Test akışında dikkat edilmesi gereken noktalar:

- Gerçek tutarlılık için `smol::Timer` yerine `cx.background_executor().timer(d)`'nin tercih edilmesi önerilir.
- `run_until_parked` ile `advance_clock`'u kombine ederken öncelikle saat (clock) ilerletilir, ardından park durumu beklenir. `VisualTestContext` `TestAppContext`'in içine deref ettiği için normal yolda `cx.background_executor.advance_clock(d)` tercih edilir; doğrudan `advance_clock(d)` yardımcısı `VisualTestAppContext` üzerinde yer alır.
- Async test için `#[gpui::test]` `async fn(cx: &mut TestAppContext)` formunu destekler; ön plan task'ları orada `cx.spawn` ile oluşturulur.
- Pencerenin gerçekten çizilmesi için `VisualTestContext::draw(...)`, `TestAppWindow::draw()` veya doğrudan `window.draw(cx)` çağrısının dönüşünü temizleyen bir pencere güncellemesi gerekebilir; `draw` bir `ArenaClearNeeded` değeri döndürür ve sonraki çizimden önce onun `clear()` çağrısıyla element arena'sının boşaltılması gerekir, aksi halde hata ayıklama sınırları veya yerleşim bilgisi üretilmez.

**Dikkat Noktaları.** Test simülasyonunda gözden kaçabilecek noktalar:

- `simulate_keystrokes` action dispatch'ini tetikler ancak keymap binding'inin kayıtlı olması gerekir; testte `cx.bind_keys([...])` çağrılmadığında beklenen action hedefe ulaşamaz.
- `run_until_parked` zamanı ilerletmez; yalnızca bekleyen future'ları yürütür. Timer beklendiğinde `advance_clock` çağrısının da yapılması gerekir.
- `dispatch_action` odak ağacında action işleyicisi bulamadığında sessizce no-op olur; view'ın gerçekten odakta olduğundan emin olunmalıdır.
...
**VisualTestContext ve VisualTestAppContext.** `VisualTestContext` tek pencereyi görsel olarak doğrulayan bağlamdır. `from_window(...)` var olan handle'dan bağlam üretir; `update(...)`, `dispatch_action(...)`, `simulate_keystrokes(...)`, `simulate_input(...)` ve fare/modifier yardımcıları seçili pencereyi yönetir; `draw(...)`, `debug_bounds(...)`, `window_title()` ve `document_path()` çizim çıktısını ve platform metadata'sını okumaya yarar. macOS test-support tarafındaki `VisualTestAppContext` ise `open_offscreen_window(...)`, `capture_screenshot(...)`, `wait_for(...)`, `wait_for_animations()`, `is_screen_capture_supported()` ve window argümanlı simülasyon metotlarıyla daha platforma yakın çalışır. Tercih edilen bağlamı test adında veya kurulumunda belirtmek, ileride test flake'lerini (kararsızlıklarını) ayıklamayı kolaylaştırır.

**Test platformu.** `TestPlatform` ve `TestDispatcher` üretim platformunun kontrollü kopyasını kurar. `new(...)`, `with_text_system(...)` ve `with_platform(...)` metin sistemi/platform varyantını seçer; dispatcher tarafındaki `advance_clock(...)`, `advance_clock_to_next_timer()`, `tick()` ve `run_until_parked()` zaman ve task kuyruğunu deterministik olarak yönetir. `allow_parking()` ve `forbid_parking()` testin beklenmeyen park noktalarını yakalamaya, `set_num_cpus(...)` CPU sayısına duyarlı kodu sınamaya yarar. `TestWindow`, `TestDisplay`, `TestAtlas`, `TestScreenCaptureSource` ve `VisualTestPlatform` platform arka ucunu taklit eder. Üretim özellikleri geliştirilirken bu tipler kullanılmaz; test koşumunun özelleştirilmesi gereken durumlarda tercih edilir.
...
- Element seçimi pencerede `Inspector` global durumu üzerinden tetiklenir.
...
- `App::register_inspector_element(...)` belirli bir element tipinin inspector panel çizimini kaydeder; altta `InspectorElementRegistry::register(...)` type id'ye göre renderer closure'ını saklar. Element seçildiğinde durum için özel arayüz (UI) çizilir.
...
- `WindowInvalidator::debug_assert_paint()`, `debug_assert_prepaint()` ve `debug_assert_paint_or_prepaint()` özel element veya pencere içi helper'ın doğru çizim fazında çağrıldığını denetler. Bu metotlar uygulama davranışı için bir koşul olarak kurgulanmamalıdır; yanlış aşamadaki `paint_*`, hitbox veya ölçüm çağrılarını debug build'de erken yakalatmak için vardır.
...
**Somut Kullanım.** Bir element görünmüyorsa, öncelikle gerçekten layout alıp almadığının test selector yardımıyla doğrulanması pratik bir yaklaşım sunar:
...
Görsel testte aynı selector üzerinden son çizilen sınırlar okunabilir:
...
`.debug()` yalnız `debug_assertions` açıkken vardır ve elementin kendi sınırına kırmızı debug çerçevesi çizer. `.debug_below()` aynı debug etkisini alt ağaçtaki uygun elementlere yayar. `.debug_selector(...)` ise test-support/test yolunda bounds haritasına anahtar yazar; release build modunda no-op olduğu için üretim davranışlarına güvenlik veya iş mantığı bağlanmamalıdır.

## Profiler

`gpui` crate'i GPUI scheduler'ının ön plan ve arka plan executor'larının çalıştırdığı task'ların başlangıç, yield ve runtime bilgilerini thread başına toplar. Bu yüzey iki katmanlıdır: ucuz istatistikler sürekli güncellenebilir, ayrıntılı trace buffer'ı ise `set_trace_enabled` ile açılır. Veriler başarım analizi, reliability log'u veya geliştirici araçları üzerinden serileştirilebilir tiplerle okunabilir.

Profiler yüzeyi `profiler` feature kapısı ile daha dar hale getirilebilir. Feature kapalıyken snapshot fonksiyonları boş veri döndürür veya no-op davranır; böylece üretim binary'sinde ayrıntılı timing buffer'ı zorunlu maliyet oluşturmaz. Reliability ve benchmark gibi ölçüm isteyen akışlarda feature açık tutulduğunda thread/task ve action zamanlamaları aynı public fonksiyonlar üzerinden okunur.
...
- `ActionStatistics` ve `ActionTiming`, action dispatch sırasında en uzun bloklayan action'ları izler. `take_action_stats()` bu global action istatistiğini alır ve tamamlanmış listeyi sıfırlar; aktif action bilgisi korunur.
...
- `SerializedThreadTaskTimings::convert(anchor, ThreadTaskTimings)` thread'in tüm `TaskTiming` dizisini serileştirir, `ThreadId` değerini deterministik hash ile `u64`'e indirir. Anchor değeri genellikle uygulama başlangıç zamanıdır (app startup time); aynı anchor kullanıldığında farklı thread snapshot'ları aynı zaman ekseni üzerinde karşılaştırılabilir.
...
**Yardımcı fonksiyonlar.** Crate kökünden yeniden dışa aktarılan thread-local yardımcılar şunlardır:
...
**Frame timing yüzeyi.** Pencere çizim süreleri task timing'den ayrı bir halka buffer'da tutulur:

- `FrameTiming` tek pencere karesi için `window_id`, ilk invalidation zamanı (`dirty_at`), coalesced invalidation sayısı, `draw_start` ve `draw_end` alanlarını taşır.
- `FrameTiming::draw_duration()` yalnız `Window::draw` içinde geçen süreyi verir.
- `FrameTiming::dirty_to_draw_duration()` ilk invalidation gözlendiyse kirlenmeden çizim bitişine kadar geçen süreyi döndürür.
- `set_frame_trace_enabled(true)` frame timing toplamayı açar; `false` değerine geçildiğinde buffer temizlenir.
- `frame_trace_enabled()` anlık bayrağı okur.
- `record_frame_timing(timing)` yalnız frame trace açıkken kayıt yapar; normal uygulama kodunda elle çağrılması beklenmez.
- `FrameTimingCollector::new()` oluşturulduğu andan sonraki frame'leri izler, `collect_unseen()` her çağrıda yalnız yeni kayıtları döndürür.

Bu yüzey, özellikle benchmark ve render regresyonu analizlerinde kullanılır. `Window::draw` frame sonunda timing kaydını üretir; `bench_renderer` gibi ölçüm yardımcıları da collector ile yalnız ilgili pencerenin frame'lerini rapora dahil eder.
...
- `set_trace_enabled` yalnız trace buffer'ını etkiler; en uzun poll/runtime istatistikleri trace kapalıyken de güncellenebilir. Bu yüzden reliability raporlarında `take_all_stats(...)` çağrısı trace açılmadan da anlamlı veriler sağlayabilir.
- `TasksIncluded::CompletedAndRunning` canlı task/action sürelerini snapshot anındaki `Instant::now()` ile hesaplar. Bu davranış reliability log'u için kullanışlıdır; geriye dönük karşılaştırmalar yapılıyorsa `OnlyCompleted` daha tutarlı sonuçlar üretir.
- `save_task_timing()` yalnızca `update_running_task(...)` sonrasında çağrılmalıdır; bu sıralama scheduler tarafından sağlanır. Uygulama kodlarında manuel task timing üretmek yerine executor/profiler köprüsünden yararlanılmalıdır.
...
- `DefaultAppearance::{Light, Dark}` `WindowAppearance` değerinden türetilir ve temel GPUI renk kümesini seçmek için tercih edilir.
...
- `window.mouse_position() -> Point<Pixels>` — işaretçinin pencere içi konumu. Bağlam menüsü ve right-click menu konumlandırmasında doğrudan tercih edilir.
...
- `window.pixel_snap(...)`, `pixel_snap_f64(...)`, `pixel_snap_point(...)`, `pixel_snap_bounds(...)` — mantıksal pikseli cihaz piksel grid'ine hizalar. İnce çizgi, indent guide ve kaplama kenarlıklarında bulanıklığı azaltmak için tercih edilir.
...
- `window.set_view_id(view_id)` — prepaint fazında dispatch veya önbellek node'una view id bağlar. Kaynak yorumunda kaldırılması planlanan düşük seviyeli bir kaçış yolu olarak işaretlidir; normal view çizim akışlarında doğrudan kullanılması önerilmez.
- `window.bounds_changed(cx)` — platform resize/move geri çağrısının yaptığı durum yenileme ve gözlemci bildirme işlemini tetikler. Platform/test altyapısı içindir; uygulama kodlarında resize simülasyonları haricinde doğrudan çağrılmaması gerekir.
...
- Test koşumunda `Application::with_platform(test_platform)` ile `TestPlatform` veya `VisualTestPlatform` enjekte edilir; `App::run` GPUI'a sahipliği devrederek olay döngüsünü yürütür.
...
- `TextRenderingMode::{PlatformDefault, Subpixel, Grayscale}` desteklenir. `PlatformDefault` metin sistemi tarafında platformun önerdiği gerçek moda çözülür; ölçüm ve paint path'inde enum'u doğrudan string ayar gibi düşünülmemelidir.
...
- `cx.drop_image(image, current_window)` — tüm pencerelerde atlas temizliği yapar. O anki pencere update edilirken `App.windows` içinden geçici olarak çıkmış olabileceği için `Some(window)` argümanı da ayrıca iletilmelidir.
- Zed/GPUI görsel önbellek bırakma geri çağrıları atlas sızıntısı olmasın diye bu API'leri kullanır; normal `img()`/`svg()` kullanımında manuel olarak çağrılması gerekmez.
...
- `window.gpu_specs()` ve özellik kapılı `window.input_latency_snapshot()` tanı ekranları veya başarım analizleri içindir; uygulama durum akışının ana veri kaynağı olarak konumlandırılmamalıdır.

**Dikkat düşük seviyeli servisleri.** Bu düşük seviyeli servislerin hatalı kullanımı beklenmedik sorunlar doğurabilir:
...
Sık başvurulan platform enum'ları ve bunların hangi alanlarda anlam taşıdığı kısaca özetlenir.
...
Element üzerinde `.cursor(CursorStyle::PointingHand)` ya da kısayol olarak `.cursor_pointer()`, `.cursor_text()`, `.cursor_grab()`, `.cursor_default()` tercih edilir.
...
`FontWeight::ALL` dizisi tüm değerleri sırasıyla taşır. Arayüz bileşenlerinde genellikle `FontWeight::SEMIBOLD` ve `FontWeight::BOLD` tercih edilmektedir.
...
`Solid`, `Dashed`. `Style::border_style` veya `paint_quad` ile iletilir.

#### `Anchor`, `Corners` ve Layer-shell `Anchor`

Anchored elementlerde kullanılan tip `gpui::Anchor`'dır: `TopLeft`, `TopRight`, `BottomLeft`, `BottomRight`, `TopCenter`, `BottomCenter`, `LeftCenter`, `RightCenter`.

`Corners<T>` farklı bir tiptir; kenarlık yarıçapı ve quad köşe yarıçapları içindir. Layer-shell modülündeki `Anchor` ise bitflag yapısındadır (`TOP | BOTTOM | LEFT | RIGHT`); anchored element `Anchor`'ı ile karıştırılmamalıdır.

#### `ResizeEdge` (`gpui` crate'i)

`Top`, `Bottom`, `Left`, `Right`, `TopLeft`, `TopRight`, `BottomLeft`, `BottomRight`. `window.start_window_resize(edge)` argümanı olarak iletilir.
...
Aşağıdaki tablolar, bu dosyada anlatılan ama ayrı başlık açılması gerekmeyen düşük seviye GPUI yüzeylerini açıkça kapsar. Amaç bu tipleri uygulama geliştiricisinin nerede görmesi gerektiğini netleştirmektir: çoğu doğrudan özellik (feature) geliştirirken değil; test koşumu, platform implementasyonu, inspector, profiler, özel element veya renderer entegrasyonu yazılırken gerekmektedir.
...
| `profiler` | modül ve crate kök reexport | Task/action timing, trace ve istatistik toplama yüzeyidir; ayrıntılı trace için `set_trace_enabled` tercih edilir. |
...
| API | Alt özellikler | Kısa anlamı |
| :-- | :-- | :-- |
| `InspectorElementId` | `path`, `instance_id` | Inspector'da seçilebilir elementin sabit kaynak yolu ve instance ayırıcısını taşır. |
...
| `ModifiersChangedEvent` | `modifiers`, `capslock`, `Deref<Target = Modifiers>` | Platform modifier state değişimini taşır; doğrudan `Modifiers` metotları çağrılabilir. |
...
| `OwnedOsMenu`, `SystemMenuType` | OS menu handle, system menu sınıfı | Platform app menu ve system menu entegrasyonlarında tercih edilir. |
...
| `FontMetrics` | `ascent`, `descent`, `line_gap`, `underline_position`, `underline_thickness`, `cap_height`, `x_height`, `bounding_box` | Typeface ölçülerini font size'a göre pixel değerlerine çeviren metrics taşıyıcısıdır. |
...
- Dolgu: `Fill` (`style`); şu anda tek varyantı `Color(Background)`'tır. Tek varyantlı bir enum olmasının nedeni gelecekte ek dolgu tipleri (örneğin örüntü tabanlı fill) eklenebilmesi için API'yi sabit tutmaktır. Solid renk dışında bir şey üretmek gerektiğinde `Background` tipinin `linear_gradient`, `pattern_slash` veya `checkerboard` kurucuları tercih edilir.
- Hata ayıklama: `DebugBelow`, yalnız `debug_assertions` altında derlenir; `debug_below` stilini özel element içerisinden okumak amacıyla global işaretçi olarak tercih edilir.

Uygulama kodlarında genellikle bu enum yapıları doğrudan oluşturulmaz; `.items_center()`, `.justify_between()`, `.flex_col()`, `.whitespace_nowrap()`, `.text_ellipsis()` gibi yardımcı metotlar tercih edilir. Özel element veya style refinement geliştiriken ham enum'lara gereksinim duyulur.

#### Geometri Yardımcıları

`geometry` genel yüzeyindeki düşük seviye yardımcılar:

- `Axis` ve `Along` — yatay/dikey eksene göre `Point`, `Size`, `Bounds` gibi tiplerden ilgili boyutu seçmek amacıyla tercih edilir.
...
Bu tipler özellikle özel yerleşim hesaplarında, canvas ve path çizimlerinde ve grid yerleşim değerlerinin programatik üretiminde tercih edilmektedir.
...
Normal uygulama kodlarında bu durum tipleri çoğunlukla doğrudan yönetilmez; `div()`, `canvas(...)`, `img(...)`, `svg()`, `list(...)`, `uniform_list(...)`, `anchored()`, `deferred(...)` ve ilgili element builder'ları bunları üretir. Özel element geliştirmelerinde `Element::request_layout`, `Element::prepaint` ve `Element::paint` dönüş değerlerinde bu taşıyıcıların benzer desenleri takip edilir.
...
Bu sayede `Modifiers` üzerindeki tüm `&self` metotları — `secondary()`, `modified()`, `number_of_modifiers()`, `is_subset_of(...)` ve `control`, `alt`, `shift`, `platform`, `function` alanları — bu dört olay üzerinde doğrudan çağrılabilir. "Keystroke, Modifiers ve Platform Bağımsız Kısayollar" başlığındaki kısayollar (`Modifiers::command_shift()` vb.) ise `Modifiers` üzerinde **inherent associated function**'dır; olay nesnesi üzerinden çağrılmaz, ayrı bir `Modifiers` üretmek için tercih edilir.

`MouseDownEvent`, `MouseUpEvent` ve `MouseMoveEvent` Deref **etmez**; yalnızca `modifiers: Modifiers` alanını sunar. Bu nedenle bu üç olayda `olay.modifiers.secondary()` yazılır; dört Deref'li olayda ise hem `olay.modifiers.secondary()` hem `olay.secondary()` çalışmaktadır. Asimetri bilinçli bir tasarım tercihidir: Deref'li dörtlü "girdinin modifier şapkası budur" semantiğini taşırken, fare press/move olayları modifier bilgisini yalnızca yan veri olarak saklar.
...
- `RenderImageParams` ve `RenderSvgParams` — renderer'a verilen rasterization parametrelerini taşır.

Uygulama seviyesinde çoğunlukla `img(source)`, `svg().path(...)`, `image_cache(retain_all(id))`, `window.use_asset(...)`, `window.paint_image(...)` ve `window.paint_svg(...)` tercih edilmektedir. Önbellek uygulamaları geliştirilirken `ImageCacheProvider -> ImageCache -> ImageCacheItem` zincirine inilir.
...
- Dispatcher ve executor: rustdoc genel yüzeyinde `Scope`, `FallibleTask`, `SchedulerLocalExecutor` ve `RunnableMeta` görünür. Buna ek olarak platform sınırında `#[doc(hidden)]` tutulan `PlatformDispatcher`, `RunnableVariant` (`Runnable<RunnableMeta>` tip takma adı) ve `TimerResolutionGuard` vardır; bunlar `target/doc/gpui/all.html` listesinde görünmez ve uygulama API'si olarak kullanılmamalıdır. Detay:
  ...
  - `SchedulerLocalExecutor` — `gpui::executor` `pub use scheduler::LocalExecutor as SchedulerLocalExecutor` yeniden dışa aktarımıdır. GPUI tarafındaki `ForegroundExecutor` bunun üzerinde bir sarmalayıcıdır; ham scheduler handle'ına `ForegroundExecutor::scheduler_executor()` çağrısıyla inilir, `BackgroundExecutor::scheduler_executor()` de paralel `scheduler::BackgroundExecutor` döner. Uygulama kodu genelde `cx.foreground_executor()` veya `cx.background_executor()` kullanır; scheduler handle yalnızca scheduler crate'iyle doğrudan etkileşim gerektiğinde çekilir.
...
- Başsız ve ekran yakalama: `PlatformHeadlessRenderer`, `scap_screen_sources(...)`. Başsız renderer `render_scene_to_image(...)` ile görüntü döndürebilir, `render_scene(...)` ile görüntüyü geri okumadan gerçek scene encoding/GPU submission akışını çalıştırabilir.
...
Bu tiplerin doğru sahibi `gpui_platform` uygulamalarıdır. tanı veya platform tanı yardımcıları üzerinden dolaylı erişim sağlanır.
...
Uygulama kodunda genellikle bu tipler elle üretilmez. `Element` uygulamaları `window.paint_quad`, `window.paint_image`, `window.paint_path`, `window.paint_layer` gibi API'ler üzerinden scene'e primitive ekler. arena yapısını yöneten `ElementArenaScope` da window içinde `pub(crate)` olduğu için uygulama kodları doğrudan bu yapıya erişmez, `AnyElement`/`Element` API'leri üzerinden çalışır.
...
Genellikle bunlara `window.text_system()`, `window.line_height()`, `window.text_style()`, `StyledText`, `TextLayout` ve `InteractiveText` üzerinden erişilir. Özel metin renderer veya editör seviyesinde ölçüm kodları yazılırken ham tiplere gereksinim duyulur.

**Metin genel metotları.** Metin alt katmanında doğrudan çağrılabilen API'ler şunlardır:
...
Profiler tipleri, `gpui::profiler` üzerinden task veya thread zamanlamalarını okumak ve serileştirmek amacıyla tercih edilir. Queue hata ve iterator tipleri `PriorityQueueSender`/`PriorityQueueReceiver` kullanıldığında görünür; `PriorityQueueReceiver::is_empty()` ise benchmark dispatcher gibi kuyruk boşluğunu bloklamadan denetlemek isteyen altyapı kodlarında kullanılır. `ReadGlobal` and `UpdateGlobal` trait'leri global durum okuma ve güncelleme kısayollarını sağlar; uygulama kodlarında doğrudan `cx.global`, `cx.read_global` veya `cx.update_global` çağrıları yeterlidir.
...
Uygulama tarafında çoğu zaman `cx.prompt(...)`, `cx.prompt_for_path(...)`, `cx.prompt_for_new_path(...)` ve `PromptBuilder` yeterlidir; özel platform veya başsız prompt davranışları geliştirilerken bu taşıyıcılara inilir.
...
`generate_list_of_all_registered_actions()` listeler. `AppCell`/`AppRef`/`AppRefMut` normal uygulama kodunun sahiplenmesi gereken tipler olmayıp; `App`, `Context<T>` ve asenkron bağlamlar (context) üzerinden çalışmak doğru yaklaşımdır.
...
- `scap_screen_sources(...)` — ekran yakalama kaynaklarını platforma göre toplar; uygulama kodlarında çoğunlukla `cx.screen_capture_sources(...)` tercih edilir.
...
- `block_on` — `pollster::block_on`; eşzamanlı köprü gerektiğinde ön plan olmayan küçük future'lar için tercih edilir.
...
Başarım ölçümü (benchmark) için GPUI, Criterion ile aynı yapıyı koruyan ince bir köprü sunar. `#[gpui::bench]` öznitelik makrosu bir ölçüm fonksiyonunu işaretler; `gpui::bench_group!` ve `gpui::bench_main!` makroları `criterion::criterion_group!` ve `criterion::criterion_main!` çağrılarını birebir sarmalar, böylece bir GPUI benchmark dosyası sıradan bir Criterion benchmark dosyasıyla aynı iskelete sahip olur. Ölçüm gövdesi içinde GPUI çalışma zamanına `BenchAppContext` ve `BenchWindowContext` ile erişilir; bunlar `App` ve `Window` yüzeyini ölçüm akışına uyarlar. Bu yüzey `bench` feature kapısı altında derlenir; `bench` aynı zamanda `test-support`, `criterion` ve frame raporlaması için gereken histogram altyapısını etkinleştirir.

`#[gpui::bench]` tek bir ölçüm fonksiyonunu argümansız çalıştırabildiği gibi input'lu Criterion ölçümleri için `inputs = EXPR` parametresini de kabul eder. `input_name = "..."`, `group = "..."` ve pozitif `sample_size = N` değerleri yalnız input'lu ölçümlerde kullanılır; bu durumda üretilen kod `criterion::BenchmarkId::new(...)` ve `bench_with_input(...)` yoluyla her input için ayrı ölçüm kaydı oluşturur.

Benchmark platformu `bench_platform(headless_renderer_factory, text_system)` ile kurulur. Bu platform her çağrıda yeni bir `TestPlatform` üretir, fakat aynı thread içinde paylaşılan çok iş parçacıklı `BenchDispatcher`'ı kullanır; böylece foreground ve background işler gerçek zamanlı production concurrency'ye yakın bir kuyrukta yürür. Text shaping dışarıdan verilen `PlatformTextSystem` üzerinden yapılır. Headless renderer factory verildiğinde benchmark pencerelerinin çizdiği sahneler gerçek sprite atlas ve platform renderer hattından geçirilir; renderer yoksa present işlemi sahneyi ölçüm dışı bırakacak şekilde atar.

`BenchAppContext::bench_iter(...)` genel workload ölçümü, `BenchAppContext::bench_renderer(...)` ise bir `Entity<V: Render>` güncellemesinden sonra çizilen frame'leri ölçmek için kullanılır. Renderer ölçüm döngüsü, güncellemeden önce `BenchDispatcher` üzerindeki hazır main task'ları çalıştırır ve güncelleme sonrası dirty pencereyi present eder; böylece önceki iterasyondan kalan main-thread işler ve çizim teslim maliyeti ölçümün içine tutarlı biçimde alınır. `BenchReport` frame bütçesi, draw süresi, dirty-to-draw süresi ve invalidation sayısı gibi rapor değerlerini toplar. Ölçüm bitiminde teardown, bekleyen timer'ları iptal eder ve iptalin serbest bıraktığı işleri boşaltarak sonraki benchmark'a taşınmasını engeller. Dispatcher iç döngüsünü elle sürmek gereken düşük seviye koşullarda `run_ready_main_tasks()`, timer temizliği için `cancel_pending_timers()` ve tanı amacıyla `debug_state()` yüzeyi bulunur.

Profiler yardımcıları `get_all_timings(...)`, `get_current_thread_timings(...)`, `take_all_stats(...)`, `take_action_stats(...)`, `set_trace_enabled(...)` ve `trace_enabled()` task/action timing toplama işlemlerinde tercih edilir. Metin yedek yardımcıları `font_name_with_fallbacks(...)` ve `font_name_with_fallbacks_shared(...)` platform font ailesi yedek adını döndürür. `swap_rgba_pa_to_bgra(...)` önceden çarpılmış RGBA byte buffer'ını platform BGRA düzenine çevirmek için renk veya bitmap alt katmanında kullanılır.
