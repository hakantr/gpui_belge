...
Benchmark platformu `bench_platform(...)` ile kurulur. Bu platform `BenchDispatcher` kullanarak foreground ve background işleri gerçek zamanlı, çok iş parçacıklı bir kuyrukta yürütür; text shaping tarafında varsayılan olarak deterministik `NoopTextSystem` kullanılır. `BenchAppContext::bench_iter(...)` genel workload ölçümü, `BenchAppContext::bench_renderer(...)` ise bir `Entity<V: Render>` güncellemesinden sonra çizilen frame'leri ölçmek için kullanılır. `BenchReport` frame bütçesi, draw süresi, dirty-to-draw süresi ve invalidation sayısı gibi rapor değerlerini toplar. Ölçüm bitiminde stale timer'ların sonraki benchmark'a taşınmaması için dispatcher pending timer durumunu temizleyebilir.
