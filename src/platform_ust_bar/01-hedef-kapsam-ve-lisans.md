...
Zed'in `platform_title_bar` ve `title_bar` crate'leri **GPL-3.0-or-later** lisansı altındadır. Bu lisans, kod gövdesinin kopyalanamayacağı anlamına gelir. Buna karşılık API imzaları, JSON sözleşmeleri ve gözlemlenebilir davranış kuralları mirror edebilirsin. Kısacası satır satır kopyalama yasaktır; ama "Zed'de şu alana basınca şu davranış çalışıyor" gözlemi yasak değildir. Bu davranışı ürünün kendi kodunda, kendi kelimelerin ve kendi yapınla yeniden kurarsın.

> **Tema rehberi ilgili bölüm ile fark:** Tema sözleşmesi alan adlarını mirror eder; yani veri şekli aynı tutarsın. Burada ise **davranış** mirror edersin. "Fareye basıldığında pencere sürüklenmeye başlar" veya "kapat butonu ana caption'ın sağ ucunda durur" gibi gözlemlenebilir davranışlar telif kapsamında olmadığı için yeniden inşa edebilirsin.
...
| `zed_actions` | GPL-3.0-or-later | Eylem karşılıkları kendi `app::actions!` makronla |
...
Zed kaynak dosyasındaki bir fonksiyon imzası mirror ediliyorsa, belge yorumunu da **kendi sözcüklerinle** yeniden yazarsın. Orijinal cümle aynen taşınmaz. Örnek:
...
Hangi yol seçilirse seçilsin, kod kopyalama veya birebir uyarlama gündeme geldiğinde `platform_title_bar` paketinin `GPL-3.0-or-later` lisanslı olduğu dikkate alman gerekir. Kararı bu bilgiyle verirsin.
