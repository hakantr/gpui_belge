...
├─────────────────────────────────────────────────────────────────┤
│  Ürün başlığı (ürün tasarımı, AppTitleBar)                      │
...
| -------- | --------------- |
| Platform kabuğu | Davranış öğrenilir, kod özgün ifadelerle yazılır (GPL-3 kod gövdesi kopyalama yasak) |
| Ürün başlığı | Tamamen ürüne aittir; Zed'in `title_bar` koduyla hiçbir ilgisi yok |
| Uygulama durumu | Tamamen ürüne aittir; trait imzası `TitleBarController` çıkış API'sidir |
...
Zed'in `platform_title_bar` ve `title_bar` crate'leri **GPL-3.0-or-later** lisansı altındadır. Bu lisans, kod gövdesinin kopyalanamayacağı anlamına gelir. Buna karşılık API imzaları, JSON sözleşmeleri ve gözlemlenebilir davranış kuralları mirror edilebilir. Kısacası satır satır kopyalama yasaktır; ama "Zed'de şu alana basınca şu davranış çalışıyor" gözlemi yasak değildir. Bu davranışın ürünün kendi kodunda, özgün ifadeler ve yapılarla yeniden kurulması gerekir.
...
| Davranışı tarif edip özgün ifadelerle implement etmek | Belge yorumunu kelime kelime taşımak |
...
| `zed_actions` | GPL-3.0-or-later | Eylem karşılıkları özel `app::actions!` makrosuyla tanımlanmalıdır. |

### Üç port yaklaşımı ve lisans sonucu

| Yaklaşım | Lisans Sonucu | Kullanım Koşulu |
| ---------- | --------------- | ----------------- |
| **1. Doğrudan `platform_title_bar` bağımlılığı** | Geliştirilen uygulama **GPL-3** olur | Sadece GPL altında dağıtılması durumunda |
| **2. Crate'i vendor'la (kaynak kodu kopyala)** | Vendor'lanan kod **hâlâ GPL-3**; geliştirilen uygulama da GPL-3 olur | Yine GPL hedefleniyorsa |
| **3. Davranış mirror'ı (kendi koddan yeniden yaz)** | Geliştirilen kod **kendi lisansına** sahip olur | **Lisans-temiz hedef için tek doğru yol** |
...
Zed kaynak dosyasındaki bir fonksiyon imzası mirror ediliyorsa, belge yorumunun da **özgün biçimde** yeniden yazılması gerekir. Orijinal cümle aynen taşınmamalıdır. Örnek:
...
2. **API imzasını "yeniden yazmak" sanıp gövdeyi kelime farkıyla kopyalamak.** `pub fn sag_pencere_kontrollerini_render_et(buton_yerlesimi, kapat_eylemi, window)` gibi eşdeğer imza ve parametreleri kullanmak parite kurmaktır; tek başına kopya sayılmaz. Buna karşılık **gövde** içindeki `match`/`if`/`loop` zincirini birebir taşımak açık bir kopyadır. Gövdenin her zaman yeniden çözülmesi; özgün kodlarla yazılması gerekir.
