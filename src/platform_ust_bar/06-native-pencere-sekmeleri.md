...
Native pencere sekmeleri, özellikle macOS'taki sistem sekmeleri, başlık çubuğunun en karmaşık parçasıdır. Bu yüzden ayrı bir aşama olarak ele alırsın. Burada kararlar tek başına verilmez; controller state'i, platform çağrıları ve drag/drop hedefleri birlikte düşünülür. Bu üç parça ayrı ayrı yazılırsa aralarında uyumsuzluk çıkması çok kolaydır.

![Native Sekme Akışı](assets/native-tabs-akisi.svg)
...
1. `WorkspaceSettings::use_system_window_tabs` ayarını izlemeye başlar; pencerelerin `tabbing_identifier` değerlerini ayar değiştikçe günceller. İzleme `cx.observe_global::<SettingsStore>(...)` ile kurulur. Subscription `.detach()` ile crate ömrü boyunca canlı tutulur.
2. Yeni açılan `Workspace` entity'leri için bir action renderer kaydeder. Kayıt `cx.observe_new(|workspace: &mut Workspace, ...|)` bloğunda yapılır ve şu dört action'ı bağlar: `ShowNextWindowTab`, `ShowPreviousWindowTab`, `MoveTabToNewWindow`, `MergeAllWindows`.

**Burada kritik bir binding farkı vardır.** Bu action'lar `workspace.register_action(...)` ile değil, **`workspace.register_action_renderer(...)`** ile bağlanır. İki API arasındaki fark hem zamanlama hem de kapsam açısından önemlidir:
...
Pratik sonuç şudur: `ShowNextWindowTab` action'ı yalnızca workspace içinde birden fazla tab açıkken çalışır. Tek tab varken çağrılsa bile etki üretmez. Çünkü runtime'da action map o frame için koşula göre yeniden kurarsın. Port hedefinde "Workspace" kavramı birebir yoksa bu desen aynen taşınamayabilir. Yine de "her render'da action handler'ları koşullu olarak yeniden bağla" prensibi korunmalıdır. Aksi halde tek tab varken bile sekme dolaşma action'ı çalışıyormuş gibi görünür ve davranışı test etmek zorlaşır.
...
| - | :-- | :-- |
| 1 | Tab üzerinde middle-click (aktif tab) | Mevcut pencere |
| 2 | Tab üzerinde middle-click (başka tab) | `item.handle.update(...)` ile o pencere |
| 3 | Tab close (X) butonu click (aktif tab) | Mevcut pencere |
| 4 | Tab close (X) butonu click (başka tab) | `item.handle.update(...)` ile o pencere |
| 5 | Right-click → "Close Tab" | `handle_right_click_action` ile tab handle'ı |
| 6 | Right-click → "Close Other Tabs" | Her diğer tab handle'ı |
...
- Middle-click close başka tab → o pencereye `CloseWindow`.
- X butonu tıklama başka tab → o pencereye `CloseWindow`.
- `handle_right_click_action` helper'ı (Close Tab/Close Other Tabs context menu).
- Tab bar dışına drop → o pencerede `move_tab_to_new_window()`.
...
Sağ tık menüsü `ui::right_click_menu(ix).trigger(...).menu(...)` builder zinciriyle kurulur. Yapı şudur:
...
Bu yapıda dört menu entry'sinin her biri ayrı bir `move |window, cx| {...}` closure'u alır. Her closure gövdesinde ortak olarak `Self::handle_right_click_action(cx, window, &tabs_clone, |tab| predicate, |window, cx| body)` çağrısı yaparsın.

Buradan ilginç bir bellek davranışı doğar: `tabs` vec'i **dört defa clone'lanır** (`tabs.clone()`, `other_tabs.clone()`, `move_tabs.clone()`, `merge_tabs.clone()`). Bunun nedeni her closure'ın kendi owned kopyasına ihtiyaç duymasıdır. Reference olarak paylaşmak closure lifetime'larıyla çakışır. Port hedefinde aynı builder kalıbı kullanabilirsin: `right_click_menu(id).trigger(trigger_fn).menu(menu_builder_fn)`. Entry ekleme işi ise `menu()` callback'i içinde `ContextMenu::build` ile yaparsın.
...
Sekme barının alt sağ köşesindeki plus butonu, tıklama anında `zed_actions::OpenRecent { create_new_window: true }` action'ını dispatch eder. Bu davranış da sabit şekilde gömülüdür. Port hedefinde bu noktanın değiştirilmesi büyük ihtimalle gerekir. Bağımsız bir uygulamada bu action genellikle `NewWindow`, `OpenWorkspace` veya `CreateDocumentWindow` gibi ürünün kendi action'ı ile değiştirilir.

İlk pencere açılışındaki native tab durumu, bu settings observer zincirinden değil, daha önce çalışan başka bir yoldan beslenir. `zed::build_window_options(...)` fonksiyonu içindeki `tabbing_identifier` alanı bu görevi üstlenir. GPUI'nin pencere bootstrap aşaması ise platformun `tab_bar_visible()` ve `tabbed_windows()` çağrılarının sonucunu doğrudan controller'a işler. Daha sonra çalışan `SystemWindowTabs::init(...)` içindeki `observe_global::<SettingsStore>` gözlemcisi, `was_use_system_window_tabs` değerini hafızasında tutar. Yalnızca ayar değiştiğinde çalışır; değer aynıysa hemen erken döner.

Toggle değeri `true` olduğunda controller yeniden başlatılır. Mevcut tüm pencerelere `"zed"` identifier'ı ve tab listesi yazılır. Toggle `false` olduğunda ise mevcut pencerelerin identifier'ı `None` yapılır; ancak controller'ın `init` fonksiyonu tekrar çağrılmaz. Bu asimetri kasıtlıdır: native tab'i devre dışı bırakmak controller'ı temizlemekle değil, pencere identifier'larını geri çekmekle olur.
...
**Burada önemli bir platform farkı vardır.** `Platform::tab_bar_visible()` trait'inin default implementasyonu `false` döner. Bu default'u **yalnızca macOS** override eder. Bunun sonucu olarak Linux ve Windows'ta yukarıdaki ilk koşulun ilk parçası daima `true` kabul edilir. Yani sekme çubuğunun görünürlüğü **tamamen `SystemWindowTabController::is_visible(...)` state'ine** bağlanır.

`is_visible` fonksiyonu `visible` alanı boşsa `false` kabul eder. Controller, `App` oluşturulurken `visible: None` ile başlar; pencere bootstrap aşamasında ise `SystemWindowTabController::init_visible(cx, window.tab_bar_visible())` çağrılır. Linux ve Windows'ta `tab_bar_visible()` default olarak `false` döndüğü için ilk pencere açıldığında controller çoğunlukla `Some(false)` durumuna geçer; sekme çubuğu gizli kalır. macOS dışındaki platformlarda sekme çubuğunun görünmesi için controller'ın `set_visible(...)` veya platform toggle callback'iyle açıkça görünür duruma alınması gerekir. Yalnızca `tab_bar_visible` çağrısını aramak yetmez. Bu noktayı atlayan bir port, "neden Linux'ta sekmeler hiç görünmüyor?" sorusuyla çok zaman kaybedebilir.

macOS'taki native tabbing için `set_tabbing_identifier(Some(...))` çağrısı yalnızca pencereye identifier yazmaz. Aynı çağrı paralel olarak `NSWindow::setAllowsAutomaticWindowTabbing:YES` fonksiyonunu da çağırır. Ters yönde, `None` değeri geldiğinde aynı global izin `NO` yapılır ve pencerenin tabbing identifier'ı `nil` olur. Bu yüzden Zed'in `SystemWindowTabs::init(cx)` içindeki settings observer'ı yalnız controller state'ini değil, macOS'un native tabbing politikasını da açıp kapatır.
...
- `merge_all_windows` ise sürükleme payload'ı üzerinden veya sağ tık "Show All Tabs" menüsünden tetiklenmez. Yalnızca `MergeAllWindows` action renderer'ı, controller'daki merge fonksiyonunu ve platform merge çağrısını birlikte tetikler. Sağ tıktaki "Show All Tabs" ise yalnız `window.toggle_window_tab_overview()` çağrısı yapar; merge işlemi değildir.

Bu farklar nedeniyle native tab portunda drag/drop davranışı yalnızca `DraggedWindowTab` alanlarını taşımakla çözülmez. Davranış aynı zamanda **olayın hangi hedefte gerçekleştiğine göre** mirror etmen gerekir. "Drop nerede oldu?" sorusunun cevabına göre üç ayrı dal vardır ve bu dalların hepsi ayrı ayrı kodlanır.

Sekme sürükleme payload tipi `DraggedWindowTab`:
...
`last_dragged_tab` alanı yalnızca geçici state'tir. Tek amacı sekmenin tab bar dışına bırakılma ihtimalini yakalamaktır. Başarılı bir `on_drop` çağrısı tamamlandığında bu alan `None` yaparsın. Aksi halde state sızıntısı oluşur.

Preview render'ı, etiket fontunu aktif tema üzerinden `ThemeSettings::ui_font` değerinden alır. Yüksekliği de `Tab::container_height(cx)` ile hesaplar. Drag ghost'u için ayrı ve sabit bir yükseklik tutulmaz; önizleme gerçek sekmeyle aynı boyda çıkar.

**Controller grup mutasyonları**. Bu fonksiyonların public imzaları yüzeyde basit görünür. Fakat gövdedeki state algoritması port hedefi için kritiktir. Aşağıdaki tablo her fonksiyonun state üzerinde tam olarak ne yaptığını gösterir:
...
Burada dikkat çekici bir nokta vardır: bu iki fonksiyonda grup key sırası `HashMap` key sırası olduğu için tutarlı bir "önce/sonra" tanımı yoktur. Kaynak kodunda bu duruma işaret eden bir "next/previous ne demek?" TODO yorumu da bulunur. Port hedefinde grup geçişleri deterministik olmalıysa bu noktada `HashMap` yerine sıralı bir yapı tercih edersin.

**Tab genişliği ölçümü** ince bir mekanizmaya dayanır. Tab bar render'ı, kullanıcıya görünmeyen bir `canvas` elementi içerir. Bu `canvas` tipi iki callback alır: `prepaint: FnOnce(Bounds, &mut Window, &mut App) -> T` ve `paint: FnOnce(Bounds, T, &mut Window, &mut App)`.

Bu kullanımda `prepaint` boş bırakılır (`|_, _, _| ()`). Asıl ölçüm **`paint`** callback'inde yaparsın. Burada `bounds.size.width / number_of_tabs as f32` formülüyle bir sekme genişliği hesaplanır. Ardından bu değer `entity.update(cx, |this, cx| { this.measured_tab_width = width; cx.notify() })` çağrısıyla state'e yazılır.
...
Burada bölme hatasına karşı bir güvence vardır. `number_of_tabs` ifadesi `tab_items.len().max(1)` ile en az 1'e clamp'lenir. Böylece sekme sayısı sıfır olduğunda bile bölme işlemi güvenli kalır. Port hedefinde aynı geri besleme döngüsü kurman gerekir. Aksi halde sekme genişliği ya `0px` çıkar ya da hiç güncellenmeden statik kalır.

**Sekme ölçüleri, close ayarı ve drop işaretleri** konularını birlikte ele alırsın. Burada gözlenmesi gereken altı detay vardır:

- Canvas ölçümünden gelen `measured_tab_width.max(rem_size * 10)` değeri `DraggedWindowTab.width` alanına yazılır. Gerçek sekme wrapper'ı ayrıca `.flex_1().min_w(rem_size * 10)` kullanır. Bu ikili yapı sayesinde hem sürükleme preview gerçekçi genişlikte çıkar hem de gerçek sekme alanı `10rem` altına düşmez.
- Dış tab bar yüksekliği `Tab::container_height(cx)`, tek sekmenin yüksekliği ise `Tab::content_height(cx)` üzerinden hesaplanır. `ui::Tab` bu iki değeri `DynamicSpacing::Base32` ve `Base32 - 1px` cinsinden üretir. Port hedefinde bu değerlere sabit `32px` yazılırsa, dinamik density değişimleri doğru takip edilemez ve UI farklı yoğunluklarda bozulur.
- `ItemSettings::close_position` ayarı `Left` ve `Right` değerlerini alır; default olarak `Right`'tır. `show_close_button` ayarı ise `Always`, `Hover` ve `Hidden` değerlerini alır; default `Hover`'dır.
- `Hidden` durumunda close icon hiç eklenmez. Diğer durumlarda kapatma alanı `.top_2().w_4().h_4()` ölçüleriyle çizilir; `Left` ayarında `.left_1()`, `Right` ayarında `.right_1()` uygularsın. `Hover` durumunda icon `visible_on_hover("tab")` modifier'ı ile yalnız sekmenin üzerine gelindiğinde görünür hâle gelir.
...
- Aynı tab grubuna ait pencerelerin hepsine tek bir tab group adı verirsin.
...
- Yeni açılan pencerelerin controller'a bildirilmesi için `SystemWindowTab::new(title, handle)` çağrısı kullanırsın.
