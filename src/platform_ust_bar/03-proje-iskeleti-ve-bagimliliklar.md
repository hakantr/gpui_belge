...
| ------- | ----------- | -------- |
| `kvs_titlebar` | `PlatformTitleBar`, platforma özgü buton tipleri, native sekmeler, `WindowControlArea` mirror'ları, port-özgü `TitleBarController` trait'i (Zed'de karşılığı yoktur) | senin lisansın |
...
| `system_window_tabs.rs` | `SystemWindowTabs`, `DraggedWindowTab`, native tab davranışı (`gpui::SystemWindowTabController` burada tanımlanmaz, yalnız kullanılır) | Evet (kararsız) |
...
# platform dekorasyon düzeninden parse ile üretilir). Yalnız port'unda
# kendi ihtiyacın doğarsa ekle.
# anyhow = "1"
...
| `anyhow` (port'a özgü, opsiyonel) | Zed'in `platform_title_bar` crate'i kullanmaz (render bileşenidir, hata yaymaz). Yalnız port'unda kendi başlatma hatalarını çağırana iletmek istersen eklersin |
