...
> **Crate Adlandırma:** Bu rehberde kullanılan `kvs_*` öneki yalnızca örnektir. Pratikte bunun yerine `uygulama_titlebar`, `cekirdek_titlebar` ya da projeye uygun başka bir ad seçilebilir. Rehberdeki kalıpların, isimden bağımsız olarak aynı şekilde uygulanması gerekir.
...
`DECISIONS.md` dosyası her Zed güncellemesinde ve her mimari karar sonrasında **güncellenir**. Bu disiplin ilk bakışta zahmetli görünebilir. Fakat aradan altı ay geçtiğinde "bu karar neden alınmıştı?" sorusunun cevabı bu günlükte duruyorsa ciddi zaman kazandırır.
...
CI iş akışına aşağıdaki adımın eklenmesi gerekir:
