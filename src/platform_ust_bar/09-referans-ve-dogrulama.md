...
| Güncelleme bildirimi | `UpdateVersion::version_tooltip_message(...)` | İpucu metnini `"Update to Version: ..."` biçiminde üretir ve `semver::Version` değerini `to_string()` ile yazar. |
| Güncelleme bildiriminin görsel kabuğu | `UpdateButton` | Beş durum için ayrı kurucu: `checking`, `downloading`, `installing`, `updated`, `errored`. İlk üçü `disabled(true)` ile gelir; render sırasında düğme `disabled` bayrağına geçer ve sınırı `colors().border` ile çizilir. `updated`/`errored` durumlarında sınır `colors().text.opacity(0.15)` üzerinden hesaplanır. Döner ikon yalnız `checking` ve `installing` durumlarında `IconName::LoadCircle` ile iki turluk dönüş yapar; `downloading` progress yoksa `Download`, progress varsa `CircularProgress`, `updated` durumunda `Download`, `errored` durumunda `Warning` ikonu kullanılır. Kaynak hata görünür metni `"Failed to Update"` biçimindedir; yerelleştirilmiş portta anlam korunarak Türkçeleştirilir. |
...
`UpdateVersion` tarafında `version_tooltip_message(...)` `semver::Version::to_string()` çıktısını kaynak arayüzünde `"Update to Version: {version}"` kalıbına sarar. `Downloading`, `Installing` ve `Updated` render kolları bu metni `UpdateButton` kurucularına `version` değişkeni olarak geçirir; `Downloading` ayrıca bilinen indirme oranını `progress: Option<f32>` olarak aktarır.
