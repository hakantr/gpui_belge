...
| Güncelleme ipucu değişmeli | `UpdateVersion` muadili bileşendeki ipucu üreticisi güncellenir; kaynak davranıştaki `"Update to Version:"` öneki ve `Version::to_string()` çıktısı korunur. |
| Güncelleme butonunun durumları ayrıştırılmalı | `UpdateButton` muadilinde `checking`, `downloading`, `installing`, `updated`, `errored` için ayrı kurucular tutulur; `checking` ve `installing` durumları döner `LoadCircle` ikonuyla, `downloading` durumu progress yoksa `Download` ikonuyla, progress varsa `CircularProgress` halkasıyla ve ilk üç durum `disabled(true)` ile işaretlenir. |
...
- Güncelleme ipucunun kaynak davranıştaki `"Update to Version:"` önekini ve `Version::to_string()` çıktısını koruyup korumadığı kontrol edilir. Port görünür metinleri Türkçeyse aynı veri ayrımı yerelleştirilmiş metinde de korunur.
- Güncelleme butonunun geçici durumları (`Checking for Zed Updates…`, `Downloading Zed Update…`, `Installing Zed Update…`) sırasında tıklamanın kapalı olduğu doğrulanır; `checking` ve `installing` durumlarında döner `LoadCircle` ikonunun her turu 2 saniyede tamamlayan sürekli (sonsuz tekrarlı) bir dönüş yaptığı, `downloading` durumunda ise progress yoksa `Download` ikonunun, progress varsa `CircularProgress` halkasının kullanıldığı kontrol edilir. Port görünür metinleri Türkçeyse bu durumlar `"Denetleniyor…"`, `"İndiriliyor…"`, `"Kuruluyor…"` gibi yazılır.
...
- Güncelleme ipucunda eski `Version:` biçimini veya elle ayrıştırılmış sürüm metnini korumak: Zed davranışı `Version::to_string()` çıktısını `"Update to Version:"` önekiyle kullanır; yerelleştirilmiş portta da aynı veri ayrımı korunur.
