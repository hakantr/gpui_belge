...
`PlatformTitleBar` yalnızca üst başlık çubuğunu üretir. Bu çubuğun etrafındaki pencere gölgesi, kenarlık, resize kenarı ve client inset yönetimi onun sorumluluğunda değildir. Zed'de bu işler `workspace::client_side_decorations(...)` fonksiyonunun oluşturduğu ayrı bir sarmal tarafından yaparsın.
...
Bu sarmal yazılmasa da başlık çubuğu görünür; temel olarak çalışır. Ancak pencere kenarı ve resize davranışı Zed ile aynı hissi vermez. Yani başlık çubuğu tek başına CSD penceresi değildir; çevresinde bu sarmalın bulunması gerekir.
