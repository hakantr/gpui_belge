...
Güncelleme ipucunun biçimi, `version_tooltip_message` fonksiyonunda oluşturulur. Fonksiyon `semver::Version` (`Version`) değerinin `to_string()` çıktısını kullanır ve kaynak metinde `"Update to Version:"` önekini korur. Nightly build metadata sürümün parçasıysa metne semver build metadata olarak dahil olur. Port hedefinde bu etiketi Türkçeleştirirken aynı veri ayrımı korunmalıdır; ayrıca uzun metnin tek satıra sığacağı varsayılmaz. `Tooltip::text` veya muadili bir mekanizmada genişlik sınırı düşünülür. Genişlik sınırı yoksa ipucu taşıp ekran kenarında okunmaz hale gelebilir.
...
Bu üç durumda butonun sınır rengi `colors().border` üzerinden gelir. `updated` ve `errored` durumlarında ise sınır `colors().text.opacity(0.15)` ile çizilir. İkonografi de aynı şekilde durum-koşullu seçilir: `Checking` ve `Installing` `IconName::LoadCircle` ile her turu 2 saniyede tamamlayan sürekli (sonsuz tekrarlı) bir dönüş animasyonu uygular; `Downloading` progress değeri yoksa durağan `Download` ikonunu, progress değeri varsa aynı ikon kutusunda `CircularProgress` halkasını kullanır; `errored` ise uyarı rengiyle `Warning` ikonunu gösterir ve kapatma tutamağı ekler.
