...
- **`window_bounds`** — Açılış pozisyonu ve boyutu. `None` verilirse GPUI display'in varsayılan bounds'unu seçer; `Some` verildiğinde `Windowed`, `Maximized` veya `Fullscreen` başlangıcı yapılır. Varsayılan bounds seçilirken baz alınan boyutlar `gpui::DEFAULT_WINDOW_SIZE: Size<Pixels>` (1536×1095, ana Zed penceresi için) ve `gpui::DEFAULT_ADDITIONAL_WINDOW_SIZE` (900×750, 6:5 oranında settings/rules library tarzı ek pencereler için) const'larıdır (`window.rs:70,74`). Standart Zed boyutları yeterli geliyorsa bu sabitler doğrudan kullanılabilir.
...
- **`focus`** — Pencerenin oluşturulduğunda odak alıp almayacağı.
...
- **`kind`** — Pencere türü: `Normal`, `PopUp`, `Floating`, `Dialog`. Linux Wayland özellik bayrağı aktifken [`LayerShell`](#614-layer-shell-ve-özel-platform-pencereleri) da kullanılabilir.
...
`WindowKind` pencerenin işletim sistemine kendini ne olarak tanıttığını belirler. Sıralama, z-order, odak alma davranışı ve taskbar'da görünme buna göre değişir.

- **`Normal`** — Ana uygulama penceresi. Taskbar/Dock'ta görünür, normal pencere kısayolları geçerlidir.
- **`PopUp`** — Diğer pencerelerin üstünde duran, geçici/bildirim amaçlı pencere. Taskbar'da görünmez. Zed bildirim pencerelerinde (çağrı daveti, kayıt bildirimi vb.) bu tür kullanılır.
- **`Floating`** — Parent pencerenin üstünde sabitlenmiş floating panel. Araç/inspector benzeri ek paneller için uygundur.
...
- **`LayerShell`** — Yalnızca Linux Wayland özellik bayrağı aktifken geçerlidir. Dock, top bar, wallpaper gibi [compositor yüzeyleri](#614-layer-shell-ve-özel-platform-pencereleri) için kullanılır.

Bildirim/popup pencerelerinde tipik `WindowOptions` kombinasyonu, işletim sisteminden bağımsız tutarlı bir görünüm üretir:
...
Env override uygulama başlatılırken verilir:
...
Bu işaretlemeler olmadan custom titlebar Windows'ta sürüklenmez veya Aero Snap çalışmaz.
...
- **Windows** — `platform_title_bar::platforms::platform_windows::WindowsWindowControls` caption butonlarını uygulama içinde çizer; her butonun `WindowControlArea` ile native hit-test alanına bağlanması gerekir (yoksa Aero Snap ve Windows snap layouts çalışmaz).
...
Bu yaklaşımın sebebi, kapatma akışında dirty buffer kontrolü, "kaydet?" onayı, workspace shutdown, keybinding ile gelen close — hepsinin **aynı yol**dan geçmesidir. Doğrudan `remove_window` çağrılırsa bu adımlar atlanır ve kullanıcı kaydedilmemiş değişiklikleri sessizce kaybedebilir.
...
- Varsayılan Linux yedeği: sağda minimize, maximize, close.
...
- **Wayland** — Compositor decoration protocol sağlamazsa, "server" isteği "client"a düşer; bu durum bazı minimal compositor'larda görülebilir.
...
- Caption butonlarının native hit-test davranışı `WindowControlArea` üzerinden `HTCLOSE`, `HTMAXBUTTON`, `HTMINBUTTON`, `HTCAPTION` olarak platform event katmanında eşlenir; Aero Snap ve snap layouts bu hit-test'lere dayanır.
...
- `window.remove_window()` — Pencereyi kapatır; Zed workspace kapatma mantığında genellikle [close action akışı](#67-kontrol-butonları-nasıl-yönetilir) tercih edilir.
...
Native tabbing davranışı [Native Window Tabs ve SystemWindowTabController](#613-native-window-tabs-ve-systemwindowtabcontroller) bölümünde açıklanır.
...
Workspace açılırken `zed::build_window_options` üzerinde şu adımlar uygulanır:
...
3. Workspace'e özgü bounds yoksa global varsayılan window bounds okunur.
4. Hiç kayıt yoksa `WindowOptions.window_bounds = None` bırakılır; GPUI platforma uygun cascade/varsayılan bounds seçer.
...
- Display UUID'si Linux/Wayland'de boş olabilir (`display.uuid().ok()` `None` döner); UUID yoksa uygulama tarafında display index'i veya çözünürlük gibi daha zayıf sinyallerle eşleme yapılır ya da restore atlanır.
...
Linux Wayland compositor'larında "layer shell" adlı bir protocol vardır; bu protocol, ekrana yapışık dock, top bar, wallpaper, overlay HUD gibi *sıradan pencere olmayan* yüzeylerin oluşturulmasına izin verir. Zed'in ana workspace pencereleri normal pencere olarak açılır; Linux Wayland özellik bayrağı aktifken `WindowKind::LayerShell(LayerShellOptions)` kullanılarak bu özel yüzeylerden biri tanımlanabilir:
...
Bu API yalnızca `#[cfg(all(target_os = "linux", feature = "wayland"))]` altında derlenir; diğer platformlarda mevcut değildir. Compositor layer-shell protocol'ünü desteklemiyorsa backend `LayerShellNotSupportedError` döndürür; uygulama tarafında bu hata yakalanıp `WindowKind::Normal` ile yedek bir pencere açılabilir.
