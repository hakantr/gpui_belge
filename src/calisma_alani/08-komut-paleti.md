...
`WorkspaceSettings::command_aliases: HashMap<String, CommandAliasTarget>` yapısı, komut takma adlarını yönetir. Kullanıcı ayarları JSON dosyasına `"command_aliases": { "ag": "search::ToggleSearch", "w": "w" }` yazıldığında, komut paleti arama sorgusu tam olarak `ag` veya `w` olduğunda bu değer hedef sorguya dönüştürülür. `CommandAliasTarget`, kayıtlı bir action adını da Vim komutu gibi serbest bir palet sorgusunu da taşıyabilir; ancak `::` deseni içeren kayıtlı olmayan string'ler şema tarafında uyarı üretmek üzere dışlanır. Bu dönüşüm fuzzy (bulanık) eşleştirme ve interceptor çağrılarından önce gerçekleşir; takma ad doğrudan bir action nesnesi üretmez, yalnızca palet sorgusunu kanonik hedefe yaklaştırır. Keymap tarafında ise `ActionName` kısıtı korunur ve bağlamalarda kayıtlı action adlarının kullanılması gerekir.
...
`workspace::workspace_settings::ActivePanelModifiers`, aktif pane etrafındaki görsel vurguyu ayarlar. `border_size: Option<f32>` aktif pane kenarlık kalınlığını taşır; `0.0` değerinde kenarlık çizilmez ve kenarlık inset olarak uygulanır. `inactive_opacity: Option<InactiveOpacity>` aktif olmayan pane içeriklerinin opaklığını tutar; değerler `[0.0, 1.0]` aralığına sıkıştırılır. Bu tip doğrudan komut paletine ait değildir, fakat `WorkspaceSettings` aynı struct içinde hem `command_aliases` hem de pane davranışı kararlarını taşıdığı için çalışma alanı ayarlarının ortak bağlamında değerlendirilir.
...
5. Arayüz `Picker::uniform_list(temsilci, window, cx).reopenable(false, cx).show_scrollbar(true)` zinciriyle kurulur. Komut paleti ek bir genişlik veya yükseklik zinciri uygulamaz; sade picker'ın standart ölçüleri `DEFAULT_MODAL_WIDTH` ve `DEFAULT_MODAL_MAX_HEIGHT` üzerinden gelir. `show_scrollbar(true)` uzun komut listelerinde modal picker içinde scrollbar'ın görünür olmasını sağlar. Başlangıç sorgusu `secici.set_query(sorgu, window, cx)` ile arama kutusuna yazılır.
...
- `WorkspaceSettings::command_aliases` tam sorgu eşleşmelerini kanonik action adı string'ine veya serbest palet sorgusuna dönüştürür.
...
| `CommandPaletteDelegate::name` | `PickerDelegate` kalıcılık anahtarını `"command palette"` olarak verir; picker ayarları bu adla saklanır. |
| `humanize_action_name` | `editor::GoToDefinition` gibi kanonik action adlarını palette okunabilir formata dönüştürür. |
...
Filtre ve interceptor mekanizmalarını kullanırken dikkat edilmesi gereken hassas noktalar şunlardır:
