...
5. Arayüz `Picker::uniform_list(temsilci, window, cx)` ile kurulur. Komut paleti ek bir genişlik veya yükseklik zinciri uygulamaz; sade picker'ın standart ölçüleri `DEFAULT_MODAL_WIDTH` ve `DEFAULT_MODAL_MAX_HEIGHT` üzerinden gelir. Başlangıç sorgusu `secici.set_query(sorgu, window, cx)` ile arama kutusuna yazılır.
