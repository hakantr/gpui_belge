...
5. Arayüz `Picker::uniform_list(temsilci, window, cx)` ile kurulur; ardından picker'a `initial_width(rems(34.))`, `minimum_results_width(rems(30.))`, `height(rems(24.))` ve `no_vertical_padding()` ayarları uygulanır. Başlangıç sorgusu `secici.set_query(sorgu, window, cx)` ile arama kutusuna yazılır.
