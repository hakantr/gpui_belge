...
- Git paneli görünüm seçeneklerindeki `GitPanelGroupBy::Staging`, listeyi Conflict, Staged ve Unstaged bölümlerine ayırır. Kısmen staged bir dosya staged ve unstaged parçalarıyla iki bölümde görünür. Staged bölümündeki başlık ve satır eylemleri unstage, Unstaged bölümündekiler stage işlemi uygular; Conflict bölümü toplu stage düğmesi göstermez. Bir girdiden diff açıldığında bölüm `StagedDiff` veya `UnstagedDiff` hedefini belirler. Ayar JSON'unda bu görünüm `git_panel.group_by: "staging"` ile seçilir; varsayılan değer `"status"` olarak kalır.
...
- `position_is_valid` alt ve yan sınırlamalarının net tanımlanması gerekir.
- `toggle_action()` action'ının önceden kaydedilmesi gerekir.
- `activation_priority()` için benzersiz bir öncelik tanımlanması gerekir.
