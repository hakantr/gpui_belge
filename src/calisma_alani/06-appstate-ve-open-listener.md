...
| Grup | API | Not |
|---|---|---|
| Varyantlar | `Agent`, `PeerId` | Public enum sözleşmesinin varyantlarıdır; davranış bu dosyadaki konu bağlamıyla okunur. |

### `AnyActiveCall`

| Grup | API | Not |
|---|---|---|
| Trait metotları 1 | `channel_id`, `client`, `create_shared_screen`, `entity`, `hang_up`, `has_remote_participants`, `is_in_room`, `is_sharing_project` | Trait sözleşmesinin implementor tarafından sağlanan public metotlarıdır. |
| Trait metotları 2 | `is_sharing_screen`, `join_channel`, `join_project`, `local_participant_is_guest`, `most_active_project`, `peer_id_for_user_in_room`, `peer_ids_with_video_tracks`, `remote_participant_for_peer_id` | Trait sözleşmesinin implementor tarafından sağlanan public metotlarıdır. |
| Trait metotları 3 | `room_id`, `room_update_completed`, `share_on_join`, `share_project`, `subscribe`, `unshare_project` | Trait sözleşmesinin implementor tarafından sağlanan public metotlarıdır. |
...
| Grup | API | Not |
|---|---|---|
| Varyantlar | `External`, `SharedProject`, `UnsharedProject` | Public enum sözleşmesinin varyantlarıdır; davranış bu dosyadaki konu bağlamıyla okunur. |
| Metotlar | `from_proto` | Builder, sorgu veya runtime çağrılarıdır; ayrıntı bu dosyadaki kullanım bağlamıyla okunur. |

### `RemoteCollaborator`

| Grup | API | Not |
|---|---|---|
| Alanlar | `location`, `participant_index`, `peer_id`, `user` | Public veri sözleşmesinin alanlarıdır; kullanım bağlamı bu dosyadaki ana açıklamayla okunur. |

### `ActiveCallEvent`

| Grup | API | Not |
|---|---|---|
| Varyantlar | `LocalScreenShareStarted`, `LocalScreenShareStopped`, `ParticipantLocationChanged`, `RemoteVideoTracksChanged`, `RoomLeft` | Public enum sözleşmesinin varyantlarıdır; davranış bu dosyadaki konu bağlamıyla okunur. |

### `OpenChannelNotesById`

| Grup | API | Not |
|---|---|---|
| Alanlar | `channel_id` | Public veri sözleşmesinin alanlarıdır; kullanım bağlamı bu dosyadaki ana açıklamayla okunur. |
