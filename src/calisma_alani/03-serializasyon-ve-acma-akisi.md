...
- `workspace_matching: WorkspaceMatching` — `None`, `MatchExact`, `MatchSubpaths`, `MatchSubdirectory`.
...
| `WorkspaceMatching` | `None`, `MatchExact`, `MatchSubpaths`, `MatchSubdirectory` seçenekleriyle mevcut workspace yeniden kullanımını yönetir. `MatchSubpaths`, worktree köklerini değil mevcut projelerin altındaki dosya ve dizinleri eşleştirir. |
...
**Proje grubu sıralaması.** Sidebar'da proje gruplarının görüntülenme sırası `MultiWorkspace` üzerindeki iki yöntemle değiştirilebilir:
...
Terminalde veya diff çıktısında görünen bir yol dizgesini worktree girişine veya arka plan çözümlemesiyle elde edilmiş mutlak yola çeviren altyapıdır. Terminal çıktısından dosya açma, git diff yolu çözümleme ve hover bağlantısı gibi akışlar bu modülü kullanır.
...
pub enum OpenTarget {
    ...
    Path(PathWithPosition, bool, /* test: OpenTargetFoundBy */),
}
...
`OpenTarget::Worktree` verilen yolun worktree girişiyle eşleştiği durumu, `OpenTarget::Path` ise arka plan çözümlemesiyle bulunan mutlak yolu temsil eder. `Path` varyantındaki `bool` değeri hedefin dizin olup olmadığını belirtir; `is_file()`, `is_dir()` ve `path()` yardımcıları varyant ayrımını çağıran taraftan saklar.
...
4. Arka plan çözümlemesi proje türüne göre seçilir. Yerel projede `BackgroundPathChecks::LocalFileSystem` dosya sistemini denetler; uzak/proje tabanlı akışta `BackgroundPathChecks::ProjectPathResolution` proje katmanının `resolve_abs_path` sonucunu kullanır.
...
- Test/test-support yardımcıları `possible_open_target_with_fs_checks(..., BackgroundPathChecks)` üzerinden arka plan çözümleme modunu açıkça seçebilir. Normal `possible_open_target` akışında yerel projeler `LocalFileSystem`, diğer proje türleri `ProjectPathResolution` moduna yönlenir.
...
- `serialized_item_kind` global bir ad alanı (namespace) gibidir; başka öğe kimlikleriyle çakıştırılmaması gerekir.
- Arama eşleşme (search match) tipinin byte offset, buffer snapshot ve token ile uyumlu tutulması gerekir; bayat match'i yeni buffer üzerinde kullanmak yanlış aralığa yönlenmesine neden olur.
- `OpenOptions::visible = None` varsayılan olarak çalışma alanına görünür worktree ekleme anlamı taşımaz; path açma davranışının dizin/dosya ayrımı için açıkça seçilmesi gerekir.
