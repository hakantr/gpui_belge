...
| `ThemeSettingsContent` | Font alanları | `ui_font_family`, `ui_font_fallbacks`, `ui_font_size`, `ui_font_features`, `ui_font_weight`, `buffer_font_weight`, `buffer_line_height`, `buffer_font_features`, `agent_ui_font_size`, `agent_buffer_font_size`, `git_commit_buffer_font_size`, `markdown_preview_font_family`, `markdown_preview_code_font_family`, `markdown_preview_font_size` font odaklı parçalardır. `markdown_preview_font_size` boşsa çalışma zamanı ayar dosyasındaki `ui_font_size` değerine düşer. |
...
| `SPECIFIC_OVERRIDES_KEYMAP_PATH` | Platforma özgü keymap override yolu | macOS için `keymaps/specific-overrides-macos.json`, diğer platformlar için `keymaps/specific-overrides.json` değerini taşır. |
