...
| `RemoteSettingsContent`, `SshConnection`, `WslConnection`, `DevContainerConnection`, `RemoteProject`, `SshPortForwardOption` | SSH, WSL ve dev container bağlantı tanımları | Uzak (remote) ayarlar, `SettingsContent.remote` flatten alanının şema sınırıdır; `dev_container_use_buildkit` alanı dev container imaj oluşturma işleminde BuildKit otomatik algılama, zorunlu açık veya klasik Docker builder seçimlerini taşır. |
