...
- `local_external_config_discovery_tasks` worktree açılırken worktree'nin üst dizinlerini kök yönünde tarayan ve oralarda `.editorconfig` arayan keşif task'larıdır.

`Editorconfig::from_str` `ConfigParser::new_buffered` üzerinden parse eder, `is_root` bayrağını okur ve section'ları toplar. Parse başarısız olursa yalnız bir `anyhow::Error` döndürür. Bu hatayı yakalayıp dosyayı `None` ile saklayan ve ondan `InvalidSettingsError::Editorconfig { message, path }`'i kuran `set_configs`'tur; böylece hatalı durum UI'da gösterilir kılınır.
...
Crate içi `EditorconfigStore::set_configs(worktree_id, path, content)` yeni ya da güncellenmiş içeriği yedirir; `content = None` o yolu kaldırır. Çağrı `Result<(), InvalidSettingsError>` döner; parse hatası varsa dosya `None` parsed haliyle saklanır ve hata yukarıya iletilir. External dosya değişim olaylarını watcher/discovery hattı `EditorconfigEvent::ExternalConfigChanged` ile yayar.
...
| `EditorconfigStore` | crate içi `set_configs`, `local_editorconfig_settings`, `discover_local_external_configs_chain`, `properties` | Worktree içi/dışı config kayıtlarını yedirir, yerel kaynak zincirini listeler, parent dizinlerde external `.editorconfig` keşfi başlatır ve path için birleşik property sonucunu üretir. |
...
Ayrı bir renk okuyucu metot yoktur; VS Code `editor.semanticTokenColorCustomizations` içindeki semantic token foreground/background renkleri işlenirken Zed renk modeline satır içi `Rgba::try_from(...)` çağrısıyla ayrıştırılır.

`VsCodeSettings::settings_content()` bu private okuyucuları kullanarak bir `SettingsContent` üretir. Dışa açık import yüzeyinin ana dönüşüm noktası budur.

**Içe aktarım akışı.** `SettingsStore::import_vscode_settings(fs, vscode_settings)` `VsCodeSettings` içeriğini kullanıcı `settings.json`'una uygular. İçeri aktarım alan alan tanımlı (deterministik) bir dönüşüm dizisinden geçer; başlıca eşlemelerden bir kısmı şunlardır:
