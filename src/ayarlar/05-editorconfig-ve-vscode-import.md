...
Crate `Rgba` parse'ı için ayrıca renk yardımcıları içerir; import modülü VS Code `editor.semanticTokenColorCustomizations` içindeki semantic token foreground/background renklerini Zed renk modeline çevirirken bu okuyucuları kullanır.
...
- `import_vscode_settings` `update_settings_file` ile aynı atomic-write yolunu kullanır; yazma sırasında store'un eski user metnine sahip olduğu varsayılır. Eş zamanlı kullanıcı düzenlemeleri sırasında değişikliği reddetmek yerine önce `update_settings_file_with_completion` üzerinden bir senkron yazma tamamlaman gerekir.
