...
GPUI tarafında asset altyapısının tek yüzeyi `gpui::AssetSource` trait'idir. Tanım `crates/gpui/src/assets.rs` içindedir ve gövdesi yalnızca iki metottan oluşur:
...
`AssetSource` implementasyonu, GPUI'ye `Application::with_assets` zinciriyle aktarılır. `crates/gpui/src/app.rs` içindeki imza:
...
`Asset` trait'i (üçüncü desenin yüzeyi) `crates/gpui/src/asset_cache.rs` içinde tanımlıdır:
