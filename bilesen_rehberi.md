...
Son düzeltme turunda bulunan ana hata, public metotları yalnızca **ada göre**
denetlemekti. Bu yaklaşım, `key_binding` adının rehberde geçmesini yeterli
sayıyor ama bu metodun `Button` üzerinde mi, `Switch` üzerinde mi, yoksa ortak
trait üzerinde mi bulunduğunu doğrulamıyordu. Doğru kural: her metot
`owner::method` veya `Trait for Type::method` çiftiyle izlenir.
