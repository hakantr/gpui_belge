...
- `components`, `styles` ve `traits` modülleri doğrudan public path değildir;
...
| Diğer component'ler | `Avatar`, `AudioStatus`, `AvatarAudioStatusIndicator`, `CollaboratorAvailability`, `AvatarAvailabilityIndicator`, `Facepile`, `EXAMPLE_FACES`, `Chip`, `DiffStat`, `Disclosure`, `GradientFade`, `Vector`, `VectorName`, `KeyBinding`, `Key`, `KeyIcon`, `KeybindingHint`, `Navigable`, `NavigableEntry` |
...
- Metotlar:
  ...
  - `ScrollbarStyle::const fn to_pixels(&self) -> Pixels`
...
- Metotlar:
  - `<T> StickyItems<T> where T: StickyCandidate + Clone + 'static,::with_decoration(mut self, decoration: impl StickyItemsDecoration + 'static) -> Self`
- Trait metotları:
...
FNR==1 { owner=""; mode=""; depth=0 }
{
  ...
  if (mode == "") {
    ...
    if (line ~ /^[[:space:]]*pub (async )?fn [A-Za-z_]/) {
    ...
    if (depth == 1) {
      if (mode == "trait" && line ~ /^[[:space:]]*(pub[[:space:]]+)?(async[[:space:]]+)?fn[[:space:]][A-Za-z_]/) {
        sig=capture_signature(line)
      ...
      if (mode == "impl" && (line ~ /^[[:space:]]+pub[[:space:]]+(async[[:space:]]+)?fn[[:space:]][A-Za-z_]/ || (owner ~ / for / && line ~ /^[[:space:]]+(async[[:space:]]+)?fn[[:space:]][A-Za-z_]/))) {
        sig=capture_signature(line)
        print FILENAME ":" FNR ":" owner " :: " sig
        depth += cap_delta
        if (depth <= 0) { owner=""; mode=""; depth=0 }
...
Bu awk akışının `rg`/`grep` ile yakalayamayacağı dört sınıfı kapsamalıdır:
